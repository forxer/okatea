...
<p class="field"><label><?php echo form::checkbox('p_users_custom_fields_enabled', 1, $aPageData['config']['users_custom_fields_enabled']) ?>
<?php _e('c_a_users_users_custom_fields_enabled') ?></label></p>

<fieldset>
	<legend><?php _e('c_a_users_Activation_of_public_pages') ?></legend>

	<p class="field"><label><?php echo form::checkbox('p_enable_login_page', 1, $aPageData['config']['users_pages']['login']) ?>
	<?php _e('c_a_users_Enable_login_page') ?></label></p>

	<p class="field"><label><?php echo form::checkbox('p_enable_register_page', 1, $aPageData['config']['users_pages']['register']) ?>
	<?php _e('c_a_users_Enable_registration_page') ?></label></p>

	<p class="field"><label><?php echo form::checkbox('p_enable_log_reg_page', 1, $aPageData['config']['users_pages']['log_reg'], '', '', (!$aPageData['config']['users_pages']['login'] || !$aPageData['config']['users_pages']['register'])) ?>
	<?php _e('c_a_users_Enable_log_reg_page') ?></label>
	<span class="note"><?php _e('c_a_users_Enable_log_reg_page_note') ?></span></p>

	<p class="field"><label><?php echo form::checkbox('p_enable_forget_password_page', 1, $aPageData['config']['users_pages']['forget_password']) ?>
	<?php _e('c_a_users_Enable_page_forgotten_password') ?></label></p>

	<p class="field"><label><?php echo form::checkbox('p_enable_profile_page', 1, $aPageData['config']['users_pages']['profile']) ?>

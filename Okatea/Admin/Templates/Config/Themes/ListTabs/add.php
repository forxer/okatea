...
# Toggle
$okt->page->toggleWithLegend('add_theme_zip_title', 'add_theme_zip_content', array(
	'cookie' => 'oktAdminAddThemeZip'
));
$okt->page->toggleWithLegend('add_theme_repo_title', 'add_theme_repo_content', array(
	'cookie' => 'oktAdminAddThemeRepo'
));
	...
	<div id="add_theme_zip_content" class="two-cols">
		<form class="col"
			...
			<fieldset>
				...
				<p class="field">
					<label for="pkg_url"><?php _e('c_a_themes_plugin_zip_file_url') ?></label>
			...
			<input type="submit" class="lazy-load"
					value="<?php _e('c_a_themes_download_plugin') ?>" />
			</p>
		</form>
		<form class="col"
			action="<?php echo $view->generateAdminUrl('config_themes') ?>"
			...
			<fieldset>
				...
				<p class="field">
					<label for="pkg_file"><?php _e('c_a_themes_plugin_zip_file') ?></label>
			...
			<?php echo $okt->page->formtoken()?>
			<input type="submit" class="lazy-load"
					value="<?php _e('c_a_themes_upload_plugin') ?>" />
			</p>
		</form>
	</div>
	...
	<?php elseif (!empty($aThemesRepositories)) : ?>
		...
		<table class="common">
			...
			<tr>
				...
				<?php echo $view->escape((isset($theme['name_'.$okt['visitor']->language]) ? $theme['name_'.$okt['visitor']->language] : $theme['name']))?>

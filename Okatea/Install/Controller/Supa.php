	...
	public function page()
	{
		...
		if ($this->request->request->has('sended'))
		{
			...
			if ($this->okt->error->isEmpty())
			{
				...
				$query = 'INSERT INTO `' . $db->prefix . 'core_users` (`id`, `username`, `group_id`, `password`) ' . 'VALUES ( 1, \'Guest\', 3, \'Guest\' );';
				
				$db->query($query);
				
				# insertion superadmin (id 2)
				$query = 'INSERT INTO `' . $db->prefix . 'core_users` (' . '`id`, `username`, `group_id`, `password`, `language`, `timezone`, `email`, `registered`, `last_visit`' . ') VALUES ( ' . '2, ' . '\'' . $db->escapeStr($aUsersData['sudo']['username']) . '\', ' . '1, ' . '\'' . $db->escapeStr(password_hash($aUsersData['sudo']['password'], PASSWORD_DEFAULT)) . '\', ' . '\'fr\', ' . '\'Europe/Paris\', ' . '\'' . $db->escapeStr($aUsersData['sudo']['email']) . '\', ' . $iCurrentTimestamp . ', ' . $iCurrentTimestamp . ' ' . ');';
				...
				if (! empty($aUsersData['admin']['username']) || ! empty($aUsersData['admin']['password']) || ! empty($aUsersData['admin']['email']))
				{
					$query = 'INSERT INTO `' . $db->prefix . 'core_users` (' . '`id`, `username`, `group_id`, `password`, `language`, `timezone`, `email`, `registered`, `last_visit`' . ') VALUES ( ' . '3, ' . '\'' . $db->escapeStr($aUsersData['admin']['username']) . '\', ' . '2, ' . '\'' . $db->escapeStr(password_hash($aUsersData['admin']['password'], PASSWORD_DEFAULT)) . '\', ' . '\'fr\', ' . '\'Europe/Paris\', ' . '\'' . $db->escapeStr($aUsersData['admin']['email']) . '\', ' . $iCurrentTimestamp . ', ' . $iCurrentTimestamp . ' ' . ');';

...
class GalleriesItems
{
	...
	public function getItemsRecordset($aParams = [], $bCountOnly = false)
	{
		...
		if ($bCountOnly)
		{
			$sQuery = 'SELECT COUNT(i.id) AS num_items ' . $this->getSqlFrom($aParams) . 'WHERE 1 ' . $sReqPlus;
		}
		else
		{
	...
	protected function getSelectFields($aParams)
	{
		$aFields = array(
			...
			'il.meta_keywords',
			'il.words',
			'gl.title AS gallery_title',
			'gl.slug AS gallery_slug',
			'g.items_tpl AS gallery_items_tpl'
		);
	...
	protected function getSqlFrom($aParams)
	{
		...
		else
		{
			$aFrom = array(
				'FROM ' . $this->t_items . ' AS i ',
				'INNER JOIN ' . $this->t_items_locales . ' AS il ON i.id=il.item_id ' . 'AND il.language=\'' . $this->db->escapeStr($aParams['language']) . '\' ',
				'LEFT OUTER JOIN ' . $this->t_galleries . ' AS g ON g.id=i.gallery_id ',
				'LEFT OUTER JOIN ' . $this->t_galleries_locales . ' AS gl ON g.id=gl.gallery_id ' . 'AND gl.language=\'' . $this->db->escapeStr($aParams['language']) . '\' '
			);
		}
	 ...
	 *
	 * @param integer $mItemId
	 *        	Identifiant numérique ou slug de l'élément.
	...
	{
		if (empty($iItemId) || $this->getItemsRecordset(array(
			'id' => $iItemId,
			'active' => 2
		))->isEmpty())
		{
	...
	{
		$query = 'SELECT * FROM ' . $this->t_items_locales . ' ' . 'WHERE item_id=' . (integer) $iItemId;
	 ...
	 *
	 * @param array $aItemData
	 *        	Le tableau de données de l'élément.
	 * @return boolean
	 */
	...
	public function regenMinImages($gallery_id = null)
	{
		...
		while ($rsItems->fetch())
		{
			...
			if (!empty($aImages['img_name']))
			{
				...
				$aImagesList = array_merge($aImages, $this->getImageUploadInstance()->buildImageInfos($rsItems->id, $aImages['img_name']));
			}

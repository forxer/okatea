...
if (! empty($_POST['sended']))
{
	try
	{
		...
		foreach ($oZip->getList() as $sFileName => $aFileInfos)
		{
			...
			$aNewImagesInfos = $okt->galleries->items->getImageUploadInstance()->buildImagesInfos($iItemId, array(
				1 => basename($sDestination)
			));
...
<form id="zip-upload-form" action="module.php" method="post"
	...
	<div class="two-cols">
		...
		<p class="field col">
			<label for="gallery_id" title="<?php _e('c_c_required_field') ?>"
				class="required"><?php _e('m_galleries_zip_gallery') ?></label> <select
		...
		<p class="field col">
			<label for="p_zip_file" title="<?php _e('c_c_required_field') ?>"
				class="required"><?php _e('m_galleries_zip_file') ?></label>
		<?php echo form::file('p_zip_file')?>
		<span class="note"><?php echo Utilities::getMaxUploadSizeNotice() ?></span>
		</p>
	</div>

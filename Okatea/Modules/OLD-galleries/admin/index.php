...
if (! empty($_REQUEST['regenerate_thumbnails']))
{
	...
	{
		$okt['flashMessages']->success(__('m_galleries_list_gallery_min_regenerated'));
...
if (! empty($_GET['switch_status']) && $okt['visitor']->checkPerm('galleries_manage'))
{
	try
	{
		...
		$okt['flashMessages']->success(__('m_galleries_list_gallery_visibility_switched'));
...
if (! empty($_GET['delete']) && $okt['visitor']->checkPerm('galleries_manage'))
{
	...
	{
		$okt['flashMessages']->success(__('m_galleries_list_gallery_deleted'));
...
<?php 
# Si on as pas de galerie à afficher
...
<?php 
# Sinon, on as des galeries à afficher
else
:
	?>
...
<?php
	...
	while ($rsGalleriesList->fetch())
	{
		...
		if ((! $rsGalleriesList->locked && $okt['visitor']->checkPerm('galleries_manage')) || $okt['visitor']->is_superadmin)
		{
			...
			if ($rsGalleriesList->active)
			{
				echo '<a href="module.php?m=galleries&amp;action=index&amp;switch_status=' . $rsGalleriesList->id . '" ' . 'title="' . sprintf(__('m_galleries_list_switch_visibility_gallery_%s'), html::escapeHTML($rsGalleriesList->name)) . '" ' . 'class="icon tick">' . __('c_c_action_visible') . '</a>';
			}
			else
			{
				echo '<a href="module.php?m=galleries&amp;action=index&amp;switch_status=' . $rsGalleriesList->id . '" ' . 'title="' . sprintf(__('m_galleries_list_switch_visibility_gallery_%s'), html::escapeHTML($rsGalleriesList->name)) . '" ' . 'class="icon cross">' . __('c_c_action_hidden_fem') . '</a>';
			}
			
			echo ' - <a href="module.php?m=galleries&amp;action=gallery&amp;gallery_id=' . $rsGalleriesList->id . '" ' . 'title="' . sprintf(__('m_galleries_list_edit_gallery_%s'), html::escapeHTML($rsGalleriesList->name)) . '" ' . 'class="icon pencil">' . __('c_c_action_edit') . '</a>';

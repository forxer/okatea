...
if (!empty($_GET['delete_image']) && !empty($aItemData['item']['id']))
{
	...
	$okt['flashMessages']->success(__('m_galleries_item_updated'));
	...
	if ($okt->galleries->items->checkPostData($aItemData))
	{
		...
		else
		{
			try
			{
				...
				$okt['flashMessages']->success(__('m_galleries_item_added'));
...
<p class="field" lang="<?php echo $aLanguage['code'] ?>">
	<label for="p_title_<?php echo $aLanguage['code'] ?>"><?php $okt['languages']->hasUniqueLanguage() ? _e('m_galleries_item_title') : printf(__('m_galleries_item_title_in_%s'),$aLanguage['title']) ?> <span
...
<p class="field" lang="<?php echo $aLanguage['code'] ?>">
	<label for="p_subtitle_<?php echo $aLanguage['code'] ?>"><?php $okt['languages']->hasUniqueLanguage() ? _e('m_galleries_item_subtitle') : printf(__('m_galleries_item_subtitle_in_%s'),$aLanguage['title']) ?> <span
		class="lang-switcher-buttons"></span></label>
...
<p class="field" lang="<?php echo $aLanguage['code'] ?>">
	<label for="p_content_<?php echo $aLanguage['code'] ?>"><?php $okt['languages']->hasUniqueLanguage() ? _e('m_galleries_item_content') : printf(__('m_galleries_item_content_in_%s'),$aLanguage['title']) ?> <span
		class="lang-switcher-buttons"></span></label>
...
<p class="field">
	<label for="p_image"><?php _e('m_galleries_item_image_file') ?></label>
...
<p class="field" lang="<?php echo $aLanguage['code'] ?>">
	<label for="p_images_title_1_<?php echo $aLanguage['code'] ?>"><?php $okt['languages']->hasUniqueLanguage() ? __('m_galleries_item_image_title') : printf(__('m_galleries_item_image_title_in_%s'), $aLanguage['title']) ?> <span
		class="lang-switcher-buttons"></span></label>
...
<p class="field" lang="<?php echo $aLanguage['code'] ?>">
	<label for="p_images_alt_1_<?php echo $aLanguage['code'] ?>"><?php $okt['languages']->hasUniqueLanguage() ? __('m_galleries_item_image_alt_text') : printf(__('m_galleries_item_image_alt_text_in_%s'), $aLanguage['title']) ?> <span
		class="lang-switcher-buttons"></span></label>
...
<p>
	<a href="<?php echo $aItemData['image']['img_url']?>" rel="item_images"
...
<p>
	<a
		...
		class="icon delete"><?php _e('m_galleries_item_delete_image') ?></a>
</p>
...
<div class="two-cols">
	<p class="field col">
		<label for="p_gallery_id" title="<?php _e('c_c_required_field') ?>"
			class="required"><?php _e('m_galleries_item_gallery')?></label> <select
			id="p_gallery_id" name="p_gallery_id">
			<?php
			while ($rsGalleriesList->fetch())
			{
				echo '<option value="' . $rsGalleriesList->id . '"' . ($aItemData['item']['gallery_id'] == $rsGalleriesList->id ? ' selected="selected"' : '') . '>' . str_repeat('&nbsp;&nbsp;&nbsp;', $rsGalleriesList->level) . '&bull; ' . html::escapeHTML($rsGalleriesList->title) . '</option>';
			}
			?>
		</select>
	</p>
	...
	<p class="field col">
		<label><?php echo form::checkbox('p_active', 1, $aItemData['item']['active']) ?> <?php _e('c_c_status_Online') ?></label>
	</p>

		<?php if (!empty($okt->galleries->config->templates['item']['usables'])) : ?>
		<p class="field col">
		<label for="p_tpl"><?php _e('m_galleries_item_tpl') ?></label>
...
<form id="item-form" action="module.php" method="post"
...
echo $aTabInfos['id']?>"><span><?php echo $aTabInfos['title'] ?></span></a></li>
	...
	<input type="submit"
			value="<?php echo !empty($aItemData['item']['id']) ? _e('c_c_action_edit') : _e('c_c_action_add'); ?>" />
	</p>
</form>

...
<?php 
# début Okatea : si il n'y a PAS de galerie à afficher on peut indiquer un message
if ($rsGalleriesList->isEmpty())
:
	?>

<p>
	<em><?php _e('m_galleries_no_gallery') ?></em>
</p>
...
<div id="galleries_list">
	...
	<?php 
# début Okatea : boucle sur la liste des galeries
	...
	<div class="gallery <?php echo $rsGalleriesList->odd_even ?>">
		...
		<h2 class="gallery-title">
			<a href="<?php echo $view->escape($rsGalleriesList->url) ?>"><?php echo $view->escape($rsGalleriesList->title) ?></a>
		</h2>
...
# début Okatea : affichage image
		...
		:
			...
			<?php else : ?>
			<p class="modal-box">
			<a href="<?php echo $rsGalleriesList->image['img_url'] ?>"
				class="modal"><img
				src="<?php echo $rsGalleriesList->image['min_url'] ?>"
				<?php echo $rsGalleriesList->image['min_attr']?>
				<?php if (isset($rsGalleriesList->image['title']) && isset($rsGalleriesList->image['title'][$okt->user->language])) : ?>

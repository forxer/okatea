...
<form action="module.php" method="post">
	...
	<p class="field">
		<label for="p_theme"><?php _e('m_lbl_pirobox_config_theme') ?></label>
	<?php echo form::select('p_theme',$aThemes, $okt->lbl_pirobox->config->theme) ?></p>

	<p class="field col">
		<label for="p_my_speed"><?php _e('m_lbl_pirobox_config_my_speed') ?></label>
	<?php echo form::text('p_my_speed', 10, 255, html::escapeHTML($okt->lbl_pirobox->config->my_speed)) ?></p>

	<p class="field col">
		<label for="p_close_speed"><?php _e('m_lbl_pirobox_config_close_speed') ?></label>
	<?php echo form::text('p_close_speed', 10, 255, html::escapeHTML($okt->lbl_pirobox->config->close_speed)) ?></p>

	<p class="field col">
		<label><?php echo form::checkbox('p_slideshow',1,$okt->lbl_pirobox->config->slideShow)?>
	<?php _e('m_lbl_pirobox_config_slideshow') ?></label>
	</p>

	<p class="field col">
		<label for="p_slideSpeed"><?php _e('m_lbl_pirobox_config_slidespeed') ?></label>

...
class Filters extends BaseFilters
{
	...
	public function setPostsParams(&$aPostsParams = [])
	{
	...
	protected function setFilterVisibility()
	{
		...
		$this->fields['active'] = array(
			...
			form::select(array(
				...
				__('c_c_All_f') => - 1
			), Helpers::getPostsStatuses(true)), $this->get_posts_params['active'], $this->getActiveClass('active'))
		);
	}
	...
	protected function setFilterCategory()
	{
		...
		$this->order_by_array[__('m_news_filters_category')] = 'rubriques';
		
		if (!isset($this->get_posts_params['category_id']))
		{
			$this->setIntFilter('category_id');
			$this->get_posts_params['category_id'] = $this->params->category_id;
		}
	...
	protected function setFilterLanguage()
	{
		...
		if (!isset($this->get_posts_params['language']))
		{
			$this->setFilter('language');
			$this->get_posts_params['language'] = $this->params->language;
		}
		...
		$this->fields['language'] = array(
			...
			__('m_news_filters_language'),
			form::select(array(
				'language',
				$this->form_id . '_language'
			), $aSelectLanguagesValues, $this->get_posts_params['language'], $this->getActiveClass('language'))
		);
	}
	...
	protected function setFilterOrderBy()
	{
		...
		switch ($this->params->order_by)
		{
			default:
			case 'created_at':
				$this->get_posts_params['order'] = 'p.created_at';
				break;
			
			case 'updated_at':
				$this->get_posts_params['order'] = 'p.updated_at';
				break;
			
			case 'title':
				$this->get_posts_params['order'] = 'pl.title';
				break;
			
			case 'rubrique':
				$this->get_posts_params['order'] = 'p.category_id';
				break;
		}
		...
		$this->get_posts_params['order_direction'] = $this->params->order_direction;

...
class Categories extends NestedTreei18n
{
	...
	public function getCategories($aParams = array(), $bCountOnly = false)
	{
		...
		if ($bCountOnly)
		{
			$sQuery = 'SELECT COUNT(r.id) AS num_rubriques ' . 'FROM ' . $this->t_categories . ' AS r ' . 'LEFT JOIN ' . $this->t_categories_locales . ' AS rl ON r.id=rl.category_id ' . 'LEFT JOIN ' . $this->t_news . ' AS p ON r.id=p.category_id ' . 'WHERE 1 ' . $sReqPlus . ' ';
		}
		else
		{
	...
	public function delCategory($iCategoryId)
	{
		...
		$aChildrens = $this->getChildren($iCategoryId);
		foreach ($aChildrens as $aChildren) {
			$this->setParentId($aChildren['id'], $aChildren['parent_id']);
		}

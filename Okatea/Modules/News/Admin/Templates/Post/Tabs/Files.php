...
<?php for ($i=1; $i<=$okt->module('News')->config->files['number']; $i++) : ?>
	...
	:
		...
		?>
			...
			<label
				for="p_files_title_<?php echo $i ?>_<?php echo $aLanguage['code'] ?>"><?php $okt->languages->unique ? printf(__('m_news_post_file_title_%s'), $i) : printf(__('m_news_post_file_title_%s_in_%s'), $i, $aLanguage['title']) ?> <span
			...
			<a href="<?php echo $aPostData['files'][$i]['url'] ?>"><img
				src="<?php echo $okt->options->public_url.'/img/media/'.$aPostData['files'][$i]['type'].'.png' ?>"
				alt="" /></a>
			...
			<p>
			<a
				href="<?php echo $view->generateUrl('News_post', array('post_id' => $aPostData['post']['id'])) ?>?delete_file=<?php echo $i ?>"

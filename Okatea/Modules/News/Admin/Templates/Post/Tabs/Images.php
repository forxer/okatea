...
<?php for ($i=1; $i<=$okt->module('News')->config->images['number']; $i++) : ?>
	...
	:
		...
		?>
				...
				<label
					for="p_images_title_<?php echo $i ?>_<?php echo $aLanguage['code'] ?>"><?php $okt['languages']->hasUniqueLanguage() ? printf(__('m_news_post_image_title_%s'), $i) : printf(__('m_news_post_image_title_%s_in_%s'), $i, $aLanguage['title']) ?> <span
			...
			<p class="field" lang="<?php echo $aLanguage['code'] ?>">
				<label
					for="p_images_alt_<?php echo $i ?>_<?php echo $aLanguage['code'] ?>"><?php $okt['languages']->hasUniqueLanguage() ? printf(__('m_news_post_image_alt_text_%s'), $i) : printf(__('m_news_post_image_alt_text_%s_in_%s'), $i, $aLanguage['title']) ?> <span
					class="lang-switcher-buttons"></span></label>

	...
	if ($okt->##module_id##->checkPostData($aItemData))
	{
		...
		if (!empty($iItemId))
		{
			...
			if ($okt->##module_id##->updItem($iItemId, $cursor, $aItemData))
			{
				...
				$okt['flashMessages']->success(__('m_##module_id##_confirm_edited'));
		...
		else
		{
			...
			if (($iItemId = $okt->##module_id##->addItem($cursor, $aItemData)) !== false)
			{
				...
				$okt['flashMessages']->success(__('m_##module_id##_confirm_added'));
		...
		<div id="tab_files">
			...
			<?php for ($i=1; $i<=$okt->##module_id##->config->files['number']; $i++) : ?>
				...
				if (!empty($aItemFiles[$i])) : ?>
					<p><a href="<?php echo $aItemFiles[$i]['url'] ?>"><img src="<?php echo $okt['public_url'].'/img/media/'.$aItemFiles[$i]['type'].'.png' ?>" alt="<?php echo $aItemFiles[$i]['filename'] ?>" /></a>

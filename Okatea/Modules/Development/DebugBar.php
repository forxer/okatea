...
class DebugBar
{
	...
	public function getHtml()
	{
		...
		return sprintf($this->getHtmlBlock(), ($this->aConfig['tabs']['super_globales'] ? $this->getSuperGlobalesPanel() : '') . ($this->aConfig['tabs']['app'] ? $this->getAppPanel() : '') . ($this->aConfig['tabs']['db'] ? $this->getDatabasePanel() : '') . ($this->aConfig['tabs']['tools'] ? $this->getToolsPanel() : ''));
	}
	...
	protected function getHtmlBlock()
	{
		...
		return '<div id="debugBar" style="display:none">
			<ul id="debugTabs">' . '<li>' . implode('</li><li>', $aItems) . '</li>' . '</ul><!-- #debugTabs -->
			<div id="debugPanel">
			%s
			</div><!-- #debugPanel -->
		</div><!-- #debugBar -->';
	...
	protected function getSuperGlobalesPanel()
	{
		...
		{
			$sListitems .= '<li><a href="#superglobal_get">Get - ' . $this->aDebugBarData['num_data']['get'] . '</a></li>';

			$sTabContent .= '<h3 id="superglobal_get">Get</h3>' . '<div><pre>' . var_export($this->okt['request']->query->all(), true) . '</pre></div>';
		}

		if ($this->aDebugBarData['num_data']['post'] > 0)
		{
			$sListitems .= '<li><a href="#superglobal_post">Post - ' . $this->aDebugBarData['num_data']['post'] . '</a></li>';

			$sTabContent .= '<h3 id="superglobal_post">Post</h3>' . '<div><pre>' . var_export($this->okt['request']->request->all(), true) . '</pre></div>';
		}

		if ($this->aDebugBarData['num_data']['cookies'] > 0)
		{
			$sListitems .= '<li><a href="#superglobal_cookies">Cookies - ' . $this->aDebugBarData['num_data']['cookies'] . '</a></li>';

			$sTabContent .= '<h3 id="superglobal_cookies">Cookies</h3>' . '<div><pre>' . var_export($this->okt['request']->cookies->all(), true) . '</pre></div>';
		}
		...
		{
			$sListitems .= '<li><a href="#superglobal_files">Files - ' . $this->aDebugBarData['num_data']['files'] . '</a></li>';

			$sTabContent .= '<h3 id="superglobal_files">Files</h3>' . '<div><pre>' . var_export($this->okt['request']->files->all(), true) . '</pre></div>';
		}
		...
		{
			$sListitems .= '<li><a href="#superglobal_server">Server - ' . $this->aDebugBarData['num_data']['server'] . '</a></li>';

			$sTabContent .= '<h3 id="superglobal_server">Server</h3>' . '<div><pre>' . var_export($this->okt['request']->server->all(), true) . '</pre></div>';
		}
	...
	protected function getAppPanel()
	{
		...
		{
			$sListitems .= '<li><a href="#app_definedVars">Variables (' . $this->aDebugBarData['num_data']['definedVars'] . ')</a></li>';

			$sTabContent .= '<h3 id="app_definedVars">Variables dans le scope global</h3>' . '<div><pre>' . var_export($this->aDebugBarData['definedVars'], true) . '</pre></div>';
		}

		if ($this->aDebugBarData['num_data']['definedConstants'] > 0)
		{
			$sListitems .= '<li><a href="#app_definedConstants">Constantes (' . $this->aDebugBarData['num_data']['definedConstants'] . ')</a></li>';

			$sTabContent .= '<h3 id="app_definedConstants">Constantes</h3>' . '<div><pre>' . var_export($this->aDebugBarData['definedConstants'], true) . '</pre></div>';
		}

		if ($this->aDebugBarData['num_data']['configVars'] > 0)
		{
			$sListitems .= '<li><a href="#app_configVars">Configuration (' . $this->aDebugBarData['num_data']['configVars'] . ')</a></li>';

			$sTabContent .= '<h3 id="app_configVars">Configuration</h3>' . '<div><pre>' . var_export($this->aDebugBarData['configVars'], true) . '</pre></div>';
		}

		if ($this->aDebugBarData['num_data']['userVars'] > 0)
		{
			$sListitems .= '<li><a href="#app_userVars">Utilisateur (' . $this->aDebugBarData['num_data']['userVars'] . ')</a></li>';

			$sTabContent .= '<h3 id="app_userVars">Variables utilisateur</h3>' . '<div><pre>' . var_export($this->aDebugBarData['userVars'], true) . '</pre></div>';
		}

		if ($this->aDebugBarData['num_data']['l10nVars'] > 0)
		{
			$sListitems .= '<li><a href="#app_l10nVars">Localisation (' . $this->aDebugBarData['num_data']['l10nVars'] . ')</a></li>';

			$sTabContent .= '<h3 id="app_l10nVars">Variables de localisation</h3>' . '<div><pre>' . var_export($this->aDebugBarData['l10nVars'], true) . '</pre></div>';
		}

		return '<div id="debugApp">' . '<ul>' . $sListitems . '</ul>' . $sTabContent . '</div><!-- #debugApp -->';
	}

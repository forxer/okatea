...
if ($okt->checkPerm('users_edit'))
{
	...
	$aEditTabs[10]['content'] =
		'<h3>'.sprintf(__('m_users_edit_informations_of_user_%s'), '<em>'.html::escapeHTML($edit_username).'</em>').'</h3>

		<form id="edit-user-form" action="module.php" method="post">
			<fieldset>
				<legend>'.__('m_users_Edit_informations').'</legend>
				<div class="three-cols">
					<p class="field col"><label for="edit_username" title="'.__('c_c_required_field').
					'" class="required">'.__('c_c_user_Username').'</label>'.
					form::text('edit_username', 40, 255, html::escapeHTML($edit_username)).'</p>

					<p class="field col"><label for="edit_email" title="'.__('c_c_required_field').
					'" class="required">'.__('c_c_Email').'</label>'.
					form::text('edit_email', 40, 255, html::escapeHTML($edit_email)).'</p>

					<p class="field col"><label for="edit_active">'.form::checkbox('edit_active', 1, $edit_active).' '.__('c_c_status_Active').'</label></p>
				</div>
				<div class="three-cols">
					<p class="field col"><label for="edit_civility">'.__('c_c_Civility').'</label>'.
					form::select('edit_civility', $aCivilities, $edit_civility).'</p>

					<p class="field col"><label for="edit_lastname">'.__('c_c_Name').'</label>'.
					form::text('edit_lastname', 40, 255, html::escapeHTML($edit_lastname)).'</p>

					<p class="field col"><label for="edit_firstname">'.__('c_c_First_name').'</label>'.
					form::text('edit_firstname', 40, 255, html::escapeHTML($edit_firstname)).'</p>
				</div>
				<div class="two-cols">
					<p class="field col"><label for="edit_language">'.__('c_c_Language').'</label>'.
					form::select('edit_language', $aLanguages, html::escapeHTML($edit_language)).'</p>

					<p class="field col"><label for="edit_timezone">'.__('c_c_Timezone').'</label>'.
					form::select('edit_timezone', dt::getZones(true,true), html::escapeHTML($edit_timezone)).'</p>
				</div>';
				...
				{
					$aEditTabs[10]['content'] .=
						'<p class="col">'.__('m_users_user_in_wait_of_validation').', <a href="module.php?m=users&amp;action=edit&amp;id='.
						$aEditPageInfos['iUserId'].'&amp;valide=1">'.__('m_users_validate_this_user').'</a>.</p>';
				}
				else
				{
					$aEditTabs[10]['content'] .=
						'<p class="field col"><label for="edit_group_id">'.__('c_c_Group').'</label>'.
						form::select('edit_group_id', $groups_array, $edit_group_id).'</p>';
				}
	...
	if ($okt->checkPerm('change_password'))
	{
		...
		<form action="module.php" method="post">
			<fieldset>
				...
				<div class="two-cols">
					<p class="field col"><label for="edit_password">'.__('c_c_user_Password').'</label>'.
					form::password('edit_password', 40, 255, html::escapeHTML($edit_password)).'</p>

					<p class="field col"><label for="edit_password_confirm">'.__('c_c_auth_confirm_password').'</label>'.
					form::password('edit_password_confirm', 40, 255, html::escapeHTML($edit_password_confirm)).'</p>
				</div>
				<div class="two-cols">
					<p class="field col"><label>'.form::checkbox('send_password_mail',1,0).' '.
					__('m_users_Alert_user_by_email').'</label></p>
				</div>
			</fieldset>
			<p>'.form::hidden('change_password',1).
			form::hidden('m','users').
			form::hidden('action','edit').
			form::hidden('id',$aEditPageInfos['iUserId']).
			Page::formtoken().

...
class module_diary extends Module
{
	...
	public function getEvents($aParams = array(), $bCountOnly = false)
	{
		...
		if ($bCountOnly)
		{
			$query = 'SELECT COUNT(id) AS num_events ' . 'FROM ' . $this->table . ' ' . 'WHERE 1 ' . $sReqPlus;
		}
		else
		{
	...
	public function regenMinImages()
	{
		...
		while ($rsEvents->fetch())
		{
			...
			foreach ($aImages as $key => $image)
			{
				...
				$aImagesList[$key] = array_merge($aImages[$key], $this->getImageUpload()->buildImageInfos($rsEvents->id, $image['img_name']));
			}
	...
	protected function buildEventSlug($title, $url, $iEventId)
	{
		...
		$query = 'SELECT slug FROM ' . $this->table . ' ' . 'WHERE slug=\'' . $this->db->escapeStr($url) . '\' ' . 'AND id <> ' . (integer) $iEventId . ' ' . 'ORDER BY slug DESC';

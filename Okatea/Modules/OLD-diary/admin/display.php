	...
	<div id="tabered">
		...
		<div id="tab_public">
			...
			<fieldset>
				...
				<div class="two-cols">
					<p class="field col">
						<label for="p_public_default_order_by">Ordre d’affichage par
							défaut sur la partie publique</label>
				<?php echo form::select('p_public_default_order_by',$field_choice_order_by,$okt->diary->config->public_default_order_by)?></p>

					<p class="field col">
						<label for="p_public_default_order_direction">Sens d’affichage par
							défaut sur la partie publique</label>
				<?php echo form::select('p_public_default_order_direction',$field_choice_order_direction,$okt->diary->config->public_default_order_direction)?></p>
					...
					<p class="field col">
						<label for="p_public_default_nb_per_page">Nombre par défaut
							d'éléments par page sur la partie publique</label>
		...
		<div id="tab_admin">
			...
			<fieldset>
				...
				<div class="two-cols">
					<p class="field col">
						<label for="p_admin_default_order_by">Ordre d’affichage par défaut
							sur l’interface d’administration</label>
				<?php echo form::select('p_admin_default_order_by',$field_choice_order_by,$okt->diary->config->admin_default_order_by)?></p>

					<p class="field col">
						<label for="p_admin_default_order_direction">Sens d’affichage par
							défaut sur l’interface d’administration</label>
				<?php echo form::select('p_admin_default_order_direction',$field_choice_order_direction,$okt->diary->config->admin_default_order_direction)?></p>
					...
					<p class="field col">
						<label for="p_admin_default_nb_per_page">Nombre par défaut
							d'éléments par page sur l’interface d’administration</label>
			...
			<fieldset>
				...
				<p class="field">
					Afficher les filtres <label>dans une boite de dialogue <?php echo form::radio(array('p_admin_filters_style'),'dialog',($okt->diary->config->admin_filters_style=='dialog'))?></label>
		...
		<div id="tab_images">
			...
			<fieldset>
				...
				<?php if ($okt->page->hasLbl()) : ?>
					<p class="field">
					<label for="p_lightbox_type">Choisissez l’interface d’affichage des
						images</label>
				...
				<p>
					Actuellement utilisé : <em><?php
				
$aChoices = array_merge(array(
					'' => 'aucune'
				), $okt->page->getLblList());
				echo $aChoices[$okt->diary->config->lightbox_type]?></em>
				</p>
				<?php else : ?>
					<p>
					<span class="icon error"></span>Il n’y a aucune interface d’affichage des images de disponible.
					<?php echo form::hidden('p_lightbox_type',0); ?></p>
				<?php endif;?>

				<p class="modal-box">
					<a class="modal" rel="test_images" title="Exemple 1"
					...
					</a> <a class="modal" rel="test_images" title="Exemple 2"
					...
					</a> <a class="modal" rel="test_images" title="Exemple 3"

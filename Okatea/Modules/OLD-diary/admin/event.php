...
<form id="event-form" action="module.php" method="post"
		...
		<div id="tab-content">
			...
			<p class="field">
				<label for="p_title" title="<?php _e('c_c_required_field') ?>"
					class="required"><?php _e('m_diary_title') ?></label>
			<?php echo form::text('p_title', 60, 255, html::escapeHTML($aEventData['title'])) ?></p>

			<p class="field">
				<label for="p_description"><?php _e('m_diary_description') ?></label>
			<?php echo form::textarea('p_description', 57, 10, $aEventData['description']) ?></p>

			<div class="two-cols">
				<p class="field col">
					<label for="p_date" title="<?php _e('c_c_required_field') ?>"
						class="required"><?php _e('m_diary_date') ?></label>
				<?php echo form::text('p_date', 20, 255, ($aEventData['date'] ? dt::dt2str('%d-%m-%Y',$aEventData['date']) : ''), 'datepicker') ?></p>

				<p class="field col">
					<label for="p_date_end"><?php _e('m_diary_date_end')?></label>
		...
		<div id="tab_images">
			...
			<?php for ($i=1; $i<=$okt->diary->config->images['number']; $i++) : ?>
				<div class="col">
					...
					<p class="field">
						<label for="p_images_<?php echo $i ?>"><?php printf(__('m_diary_image_n'), $i) ?></label>
...
# il y a une image ?
				...
				:
					...
					?>
						...
						<p class="field">
						<label for="p_images_alt_<?php echo $i ?>"><?php printf(__('m_diary_alt_image_n'), $i) ?></label>
						<?php echo form::text('p_images_alt_'.$i,40,255,html::escapeHTML($cur_image_alt)) ?></p>

					<p>
						<a href="<?php echo $aEventImages[$i]['img_url']?>"
							rel="event_images"
							title="<?php echo html::escapeHTML($aEventData['title']) ?>, <?php printf(__('m_diary_image_n'), $i) ?>"
							class="modal"><img src="<?php echo $cur_image_url ?>"
							<?php echo $cur_image_attr ?> alt="" /></a>
					</p>
					...
					echo $iEventId?>&amp;delete_image=<?php echo $i ?>"
							...
							class="icon delete"><?php _e('m_diary_delete_this_image') ?></a>
					</p>
		...
		<div id="tab_files">
			...
			<?php for ($i=1; $i<=$okt->diary->config->files['number']; $i++) : ?>
				<div class="col">
					<p class="field">
						<label for="p_files_<?php echo $i ?>"><?php printf(__('m_diary_file_n'), $i) ?></label>
...
# il y a un fichier ?
				...
				:
					...
					echo $iEventId?>&amp;delete_file=<?php echo $i ?>"
							...
							class="icon delete"><?php _e('m_diary_delete_this_file') ?></a>
					</p>
				<?php endif; ?>
				</div>
			<?php endfor; ?>
		...
		<div id="tab-options">
			...
			<div class="two-cols">
				<p class="field col">
					<label><?php echo form::checkbox('p_visibility', 1, $aEventData['visibility']) ?> Visible</label>
				</p>

				<?php if ($okt->diary->config->fields['disponibility']) : ?>
					<p class="field col">
					<label for="p_disponibility"
						<?php if ($okt->diary->config->fields['disponibility'] == 2) { echo ' title="'.__('c_c_required_field').'" class="required"'; } ?>><?php _e('m_diary_disponibility') ?></label>
				...
				<?php if ($okt->diary->config->fields['color']) : ?>
					<p class="field col">
					<label for="p_color"
						<?php if ($okt->diary->config->fields['color'] == 2) { echo ' title="'.__('c_c_required_field').'" class="required"'; } ?>><?php _e('m_diary_color') ?></label>
	...
	<input type="submit"
			value="<?php echo !empty($iEventId) ? __('c_c_action_edit') : __('c_c_action_add'); ?>" />
	</p>
</form>

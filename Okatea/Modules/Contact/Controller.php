...
class Controller extends BaseController
{
	...
	public function contactPage()
	{
		...
		while ($this->okt->module('Contact')->rsFields->fetch())
		{
			switch ($this->okt->module('Contact')->rsFields->type)
			{
				...
				case 3: # Menu déroulant
					$this->okt->module('Contact')->aPostedData[$this->okt->module('Contact')->rsFields->id] = isset($_REQUEST[$this->okt->module('Contact')->rsFields->html_id]) ? $_REQUEST[$this->okt->module('Contact')->rsFields->html_id] : '';
					break;
		...
		if (! empty($_POST['send']))
		{
			...
			if ($this->okt->error->isEmpty())
			{
				...
				if ($this->okt->module('Contact')->config->from_to == 'website')
				{
					...
					$oMail->message->setReplyTo($this->okt->module('Contact')
						->getReplyTo());
				}
				else
				{
					$oMail->message->setFrom($this->okt->module('Contact')
						->getFromTo());
				}
				
				# sujet
				$oMail->message->setSubject($this->okt->module('Contact')
					->getSubject());
				
				# corps du message
				$oMail->message->setBody($this->okt->module('Contact')
					->getBody());
				
				# destinataires
				$oMail->message->setTo($this->okt->module('Contact')
					->getRecipientsTo());
		...
		# title tag du module
		$this->page->addTitleTag($this->okt->module('Contact')
			->getTitle());
		...
		# titre de la page
		$this->page->setTitle($this->okt->module('Contact')
			->getName());
		
		# titre SEO de la page
		$this->page->setTitleSeo($this->okt->module('Contact')
			->getNameSeo());

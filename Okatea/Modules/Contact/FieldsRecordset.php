	...
	public function getHtmlField()
	{
		...
		switch($this->type)
		{
			...
			case 4 :
				...
				{
					$str .= '<li><label>'.form::radio(array($this->html_id, $this->html_id.'_'.$k), $k, ($k == $this->okt->module('Contact')->aPostedData[$this->id])).
					Escaper::html($v).'</label></li>';
				}

				$return = '<p class="field" id="'.$this->html_id.'-wrapper">'.
				'<span class="fake-label">'.Escaper::html($this->title).'</span></p>'.

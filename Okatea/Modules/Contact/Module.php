...
class Module extends BaseModule
{
	...
	protected function prepend_admin()
	{
		...
		if ($this->okt->page->display_menu)
		{
			...
			$this->okt->page->mainMenu->add($this->getName(), $this->okt->adminRouter->generate('Contact_index'), $this->okt->request->attributes->get('_route') === 'Contact_index', 2000, $this->okt->checkPerm('contact_usage'), null, $this->okt->page->contactSubMenu, $this->okt->options->public_url . '/modules/' . $this->id() . '/module_icon.png');
			$this->okt->page->contactSubMenu->add(__('m_contact_menu_recipients'), $this->okt->adminRouter->generate('Contact_index'), $this->okt->request->attributes->get('_route') === 'Contact_index', 10, $this->okt->checkPerm('contact_usage') && $this->okt->checkPerm('contact_recipients'));
			$this->okt->page->contactSubMenu->add(__('m_contact_menu_fields'), $this->okt->adminRouter->generate('Contact_fields'), in_array($this->okt->request->attributes->get('_route'), array(
				...
				'Contact_field'
			)), 20, $this->okt->checkPerm('contact_usage') && $this->okt->checkPerm('contact_fields'));
			$this->okt->page->contactSubMenu->add(__('m_contact_menu_configuration'), $this->okt->adminRouter->generate('Contact_config'), $this->okt->request->attributes->get('_route') === 'Contact_config', 30, $this->okt->checkPerm('contact_usage') && $this->okt->checkPerm('contact_config'));
		}
	}

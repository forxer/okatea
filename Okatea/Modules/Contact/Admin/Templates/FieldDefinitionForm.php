...
<div class="two-cols">
	...
	<p class="field col" lang="<?php echo $aLanguage['code'] ?>"><label for="field_title_<?php echo $aLanguage['code'] ?>" title="<?php _e('c_c_required_field') ?>" class="required"><?php $okt->languages->unique ? _e('m_contact_field_title') : printf(__('m_contact_field_title_in_%s'), $aLanguage['title']) ?> <span class="lang-switcher-buttons"></span></label>
	...
	<p class="field col"><label for="field_html_id"><?php _e('m_contact_field_html_id')?></label>
	<?php echo form::text('field_html_id', 60, 255, $view->escape($aFieldData['html_id']))?></p>
</div>
<div class="two-cols">
	<p class="field col"><label for="field_type" title="<?php _e('c_c_required_field') ?>" class="required"><?php _e('m_contact_field_type') ?></label>
...
<div class="two-cols">
	<?php foreach ($okt->languages->list as $aLanguage) : ?>
	<p class="field col" lang="<?php echo $aLanguage['code'] ?>"><label for="field_description_<?php echo $aLanguage['code'] ?>"><?php $okt->languages->unique ? _e('m_contact_field_description') : printf(__('m_contact_field_description_in_%s'), $aLanguage['title']) ?> <span class="lang-switcher-buttons"></span></label>

...
class Fields
{
	...
	protected static $aUnDisablableFields = array(
		4,
		6,
		7
	);
	...
	public function getFields(array $params = array())
	{
		...
		$query = 'SELECT f.id, f.status, f.type, f.ord, f.html_id, fl.title, fl.description, fl.value ' . 'FROM ' . $this->t_fields . ' f ' . 'LEFT JOIN ' . $this->t_fields_locales . ' AS fl ON fl.field_id=f.id ' . 'WHERE 1 ' . $reqPlus . 'ORDER BY ord ASC ';
	...
	{
		$query = 'SELECT language, title, value, description ' . 'FROM ' . $this->t_fields_locales . ' ' . 'WHERE field_id=' . (integer) $iFieldId;

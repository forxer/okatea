...
# -- CORE TRIGGER : publicModuleContactTplFormBottom
	...
	<p class="submit-wrapper">
		<input type="submit" value="<?php _e('m_contact_send'); ?>"
			name="send" id="submit-contact-form" />
	</p>
...
# début Okatea : affichage du numéro de téléphone
	...
	:
		?>
	<p>
		<span class="label"><abbr title="<?php _e('c_c_Phone') ?>"><?php _e('m_contact_tel') ?></abbr>
...
# début Okatea : affichage du numéro de fax
	...
	:
		?>
	<p>
		<span class="label"><?php _e('m_contact_fax') ?> :</span> <?php echo $view->escape($okt->config->address['fax']) ?></p>
	<?php endif; # fin Okatea : affichage du numéro de fax ?>

	<?php # début Okatea : affichage de l'image de l'email ?>
	<p>
		<strong><?php _e('m_contact_email'); ?> : </strong><img
			src="<?php echo $okt->module('Contact')->genImgMail(); ?>"
			alt="E-mail <?php echo $view->escape($okt->config->company['name']) ?>">
	</p>
...
# début Okatea : lien pour afficher le plan dans UI dialog
		...
		:
			?>
<p id="google_map_link_wrapper">
	<a id="google_map_link" href="#google_map"><?php _e('m_contact_access_map') ?></a>
</p>

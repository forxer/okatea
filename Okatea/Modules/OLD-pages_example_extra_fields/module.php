...
class module_pages_example_extra_fields extends Module
{
	...
	protected function prepend()
	{
		...
		$this->okt->module('Pages')->triggers->registerTrigger('getPagesSelectFields', array(
			$this,
			'getPagesSelectFields'
		));
		$this->okt->module('Pages')->triggers->registerTrigger('adminPostInit', array(
			$this,
			'adminPostInit'
		));
		$this->okt->module('Pages')->triggers->registerTrigger('adminPopulateData', array(
			$this,
			'adminPopulateData'
		));
		$this->okt->module('Pages')->triggers->registerTrigger('checkPostData', array(
			$this,
			'checkPostData'
		));
		$this->okt->module('Pages')->triggers->registerTrigger('beforePageUpdate', array(
			$this,
			'beforePageUpdate'
		));
		$this->okt->module('Pages')->triggers->registerTrigger('beforePageCreate', array(
			$this,
			'beforePageCreate'
		));
		$this->okt->module('Pages')->triggers->registerTrigger('adminPostBuildTabs', array(
			$this,
			'adminPostBuildTabs'
		));
	}

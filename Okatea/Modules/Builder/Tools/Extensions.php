...
class Extensions extends BaseTools
{
	...
	public function process()
	{
		...
		$ExtensionsFinder = (new Finder())->directories()
			->in($this->sTempDir)
			->depth('== 0');
		...
		foreach ($ExtensionsFinder as $extension)
		{
			...
			{
				$archiver = (new Archiver())->make($this->sPackagesDir . '/' . $sExtensionId . '.zip')
					->add($sExtensionPath)
					->close();
				
				$aExtensionInfos = require $sExtensionPath . '/_define.php';
				
				$this->aRepositoryInfos[$sExtensionId] = array_merge(array(
					'id' => $sExtensionId,
				...
				{
					$LocalesFinder = (new Finder())->directories()
		...
		file_put_contents($this->sPackagesDir . '/index.json', json_encode($this->aRepositoryInfos, JSON_PRETTY_PRINT));
	}
}

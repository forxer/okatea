...
if (! empty($_POST['form_sent']))
{
	...
	if ($okt->error->isEmpty())
	{
		...
		http::redirect('module.php?m=guestbook&action=config');
	}
}
	...
	<div id="tabered">
		...
		<div id="tab-fields">
			<fieldset>
				...
				<div class="three-cols">
					<p class="col field">
						<label for="p_chp_nom"><?php _e('c_c_Name')?></label>
					<?php echo form::select('p_chp_nom', $aFieldChoices, $p_chp_nom) ?></p>

					<p class="col field">
						<label for="p_chp_mail"><?php _e('c_c_Email')?></label>
					<?php echo form::select('p_chp_mail', $aFieldChoices, $p_chp_mail) ?></p>

					<p class="col field">
						<label for="p_chp_language"><?php _e('c_c_Language')?></label>
					<?php echo form::select('p_chp_language', $aLanguageFieldChoices, $p_chp_language) ?></p>
				</div>
				<div class="three-cols">
					<p class="col field">
						<label for="p_chp_url"><abbr title="Uniform Resource Locator">URL</abbr></label>
					<?php echo form::select('p_chp_url', $aFieldChoices, $p_chp_url) ?></p>

					<p class="col field">
						<label for="p_chp_note"><?php _e('m_guestbook_note')?></label>
		...
		<div id="tab-advanced">
			<?php if ($okt->page->hasCaptcha()) : ?>
				<p class="field">
				<label for="p_captcha"><?php _e('m_guestbook_Captcha')?></label>
			...
			<p class="field">
				<span class="fake-label"><?php _e('m_guestbook_Validation_before_publication')?></span>
				<label><?php echo form::radio(array('p_validation'), 1, $p_validation) ?> <?php _e('c_c_Yes')?></label>
				<label><?php echo form::radio(array('p_validation'), 0, !$p_validation) ?> <?php _e('c_c_No')?></label>
			</p>

			<p class="field">
				<label for="p_emails_list"><?php _e('m_guestbook_Addresses_separated_by_a_comma')?></label>
			<?php echo form::textarea('p_emails_list', 57, 3, $p_emails_list) ?></p>

			<p class="field">
				<label for="p_autodelete_spam">suppression automatique du SPAM au
					bout de combien de jour ? (0 pour désactiver)</label>
		...
		<div id="tab_seo">
			...
			<fieldset>
				...
				<p class="field" lang="<?php echo $aLanguage['code'] ?>">
					<label for="p_title_<?php echo $aLanguage['code'] ?>"><?php _e('c_c_seo_module_title_tag') ?><span
						class="lang-switcher-buttons"></span></label>

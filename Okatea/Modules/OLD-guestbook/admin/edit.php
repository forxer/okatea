...
$signature = $okt->guestbook->getSig(array(
	'id' => $sig_id
));
...
# début champ langue
	...
	<?php if ($okt->guestbook->config->chp_nom) : ?>
		<p class="field">
			<label for="nom"><?php _e('m_guestbook_full_name')?></label>
		<?php echo form::text('nom', 60, 255, html::escapeHTML($aSigData['nom'])); ?></p>
	<?php endif; ?>

	<?php if ($okt->guestbook->config->chp_mail) : ?>
		<p class="field">
			<label for="email"><?php _e('m_guestbook_email')?></label>
		<?php echo form::text('email', 60, 255, html::escapeHTML($aSigData['email'])); ?></p>
	<?php endif; ?>

	<?php if ($okt->guestbook->config->chp_url) : ?>
		<p class="field">
			<label for="url"><abbr title="Uniform Resource Locator" lang="en">URL</abbr>
				site</label>
		<?php echo form::text('url', 60, 255, html::escapeHTML($aSigData['url'])); ?></p>
	<?php endif; ?>

		<p class="field">
			<span class="fake-label"><?php _e('c_c_Date')?> :</span> <span
				class="fake-input"><?php echo $aSigData['date']; ?></span>
		</p>

	<?php if ($okt->guestbook->config->chp_note) : ?>
		<p class="field">
			<label for="note"><?php _e('m_guestbook_note')?></label>
		<?php echo form::text('note', 3, 2, ceil($aSigData['note'])); ?>/20</p>
	<?php endif; ?>

		<p>
			<label for="msg"><?php _e('m_guestbook_message')?></label>
		</p>
		<p><?php echo form::textarea('msg', 57, 10, html::escapeHTML($aSigData['message'])); ?></p>

	<?php if ($okt->guestbook->config->validation) : ?>
		<p class="field">
			<span class="fake-label"><?php _e('m_guestbook_Valid') ?></span> <label><input
				type="radio" name="visible" id="visible" value="1"
				<?php if ($aSigData['visible'] == 1) echo "checked" ?> /> Oui</label>
			<label><input type="radio" name="visible" id="visible" value="0"
				<?php if ($aSigData['visible'] == 0) echo "checked" ?> /> Non</label>
		</p>
	<?php endif; ?>
	</fieldset>
	...
	<input type="submit" name="modifier"
			value="<?php _e('c_c_action_Edit') ?>" /> <a
			href="module.php?m=guestbook&amp;action=save&amp;do=supp&amp;id=<?php
...
echo $url_params?>"
			...
			_e('m_guestbook_Confirm_signature_deletion')?>')"
			class="icon cross"><?php
			_e('c_c_action_Delete')?></a> <a
			href="module.php?m=guestbook&amp;action=index<?php echo $url_params ?>"
			class="icon arrow_turn_left"><?php
			_e('c_c_action_Go_back')?></a>
	</p>

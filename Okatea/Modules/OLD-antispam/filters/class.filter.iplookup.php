...
class oktFilterIpLookup extends oktSpamFilter
{
	...
	public function gui($url)
	{
		...
		$res .= '<form action="' . html::escapeURL($url) . '" method="post">' . '<fieldset><legend>' . __('m_antispam_IP_Lookup_servers') . '</legend>' . '<p>' . __('m_antispam_Add_coma_separated_list_of_servers') . '</p>' . '<p>' . form::textarea('bls', 40, 3, html::escapeHTML($bls), 'maximal') . '</p>' . '<p><input type="submit" value="' . __('c_c_action_Save') . '" />' . $this->okt->formNonce() . '</p>' . '</fieldset>' . '</form>';

...
class oktFilterIP extends oktSpamFilter
{
	...
	public function gui($url)
	{
		...
		if (! empty($_POST['addip']))
		{
			try
			{
				...
				$okt['flashMessages']->success(__('m_antispam_IP_successfully_added'));
		...
		if (! empty($_POST['delip']) && is_array($_POST['delip']))
		{
			try
			{
				...
				$okt['flashMessages']->success(__('m_antispam_IP_successfully_removed'));
		...
		$res .= $this->displayForms($url, 'black', __('m_antispam_Blacklist')) . $this->displayForms($url, 'white', __('m_antispam_Whitelist'));
	...
	private function displayForms($url, $type, $title)
	{
		...
		'<form action="' . html::escapeURL($url) . '" method="post">' . '<fieldset><legend>' . __('m_antispam_Add_IP_address') . '</legend><p>' . form::hidden(array(
			'ip_type'
		), $type) . form::text(array(
			'addip'
		), 18, 255) . ' ';
		...
		else
		{
			...
			while ($rs->fetch())
			{
				...
				$res .= '<p style="' . $p_style . '"><label class="classic">' . form::checkbox(array(
					'delip[]'
				), $rs->rule_id, false) . ' ' . html::escapeHTML($pattern) . '</label></p>';
			}
			$res .= '</div>' . '<p><input type="submit" value="' . __('c_c_action_Delete') . '"/>' . Page::formtoken() . form::hidden(array(
				'ip_type'
			), $type) . '</p>' . '</fieldset></form>';
		}
	...
	{
		$strReq = 'SELECT rule_id, rule_type, rule_content ' . 'FROM ' . $this->table . ' ' . "WHERE rule_type = '" . $this->db->escapeStr($type) . "' " . 'ORDER BY rule_content ASC ';
	...
	{
		$strReq = 'SELECT * FROM ' . $this->table . ' ' . "WHERE rule_type = '" . $this->db->escapeStr($type) . "' " . "AND rule_content LIKE '%:" . (integer) $ip . ":" . (integer) $mask . "' ";
	...
	{
		$strReq = 'SELECT DISTINCT(rule_content) ' . 'FROM ' . $this->table . ' ' . "WHERE rule_type = '" . $this->db->escapeStr($type) . "' " . 'ORDER BY rule_content ASC ';

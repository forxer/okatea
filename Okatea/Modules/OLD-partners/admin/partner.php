...
if (! empty($_POST['sended']))
{
	...
	if (! empty($partner_id))
	{
		...
		$okt['flashMessages']->success(__('m_partners_updated'));
	...
	else
	{
		...
		$okt['flashMessages']->success(__('m_partners_added'));
...
<form action="module.php" method="post" id="partners-form"
	...
	<div id="tabered">
		...
		<div id="tab-add-partner">
			...
			<p class="field">
				<label for="p_name"
					<?php if($okt->partners->config->chp_name == 2) :?>
					class="required" <?php endif;?>><?php _e('c_c_Name') ?></label>
			...
			<?php foreach ($okt['languages']->list as $aLanguage) : ?>
				...
				<label for="p_url_<?php echo $aLanguage['code'] ?>"
					...
					<span class="note"><?php _e('m_partners_url_example') ?></span>
			</p>
				<?php endif; ?>
			...
			<div class="two-cols">
				...
				<p class="field col">
					<label for="p_category_id"><?php _e('m_partners_Category')?></label>
					<select id="p_category_id" name="p_category_id">
						<option value="0"></option>
					<?php
					while ($rsCategories->fetch())
					{
						echo '<option value="' . $rsCategories->id . '"' . ($p_category_id == $rsCategories->id ? ' selected="selected"' : '') . '>' . str_repeat('&nbsp;&nbsp;', $rsCategories->level) . '&bull; ' . html::escapeHTML($rsCategories->name) . '</option>';
					}
					?>
				</select>
				</p>
		...
		<?php if($okt->partners->config->chp_logo > 0) : ?>
			...
			<div class="two-cols modal-box">
				<div class="col">
					...
					<p class="field">
						<label for="p_images_1"><?php printf(__('m_partners_logo')) ?></label>
...
# il y a une image ?
			...
			:
				...
				?>
							...
							<p class="field">
						<label for="p_images_alt_1"><?php printf(__('m_partners_alternative_text')) ?></label>
							<?php echo form::text('p_images_alt_1',40,255,html::escapeHTML($logo_alt)) ?></p>

					<p>
						<a href="<?php echo $aPartnerLogoInfos['img_url']?>"
							rel="partner_logo"
							title="<?php echo html::escapeHTML($logo_alt) ?>" class="modal"><img
							src="<?php echo $logo_url ?>" <?php echo $logo_attr?>
							alt="<?php echo html::escapeHTML($logo_alt) ?>" /></a>
					</p>

					<p>
						<a
							href="module.php?m=partners&amp;action=edit&amp;partner_id=<?php
				echo $partner_id?>&amp;delete_image=1"
							class="icon delete"><?php _e('m_partners_delete_logo')?></a>
					</p>
	...
	<input type="submit"
			value="<?php echo !empty($partner_id) ? _e('c_c_action_edit') : _e('c_c_action_add'); ?>" />
	</p>
</form>

...
if (! empty($_GET['delete']))
{
	...
	{
		$okt['flashMessages']->success(__('m_partners_category_deleted'));
...
if (! empty($_POST['add_category']))
{
	...
	if ($okt->error->isEmpty())
	{
		...
		{
			$okt['flashMessages']->success(__('m_partners_category_added'));
...
if (! empty($_POST['edit_category']) && $category_id)
{
	...
	if ($okt->error->isEmpty())
	{
		...
		{
			$okt['flashMessages']->success(__('m_partners_category_edited'));
...
if (! empty($_POST['ordered']) && ! empty($order))
{
	...
	$okt['flashMessages']->success(__('m_partners_category_order_edited'));
...
$okt->page->js->addReady('
	$("#sortable").sortable({
		...
		update: function(event, ui) {
			...
			$.ajax({
				data: result,
				url: "' . $okt['modules_url'] . '/partners/service_ordering_categories.php",
...
<div id="add_category">
	<form action="module.php" method="post" id="add-category-form">
		<div class="tabered">
			...
			<div id="tab-parent">
				<p class="field">
					<label for="add_category_parent"><?php __('m_partners_Relative') ?></label>
...
<?php
	...
	while ($categories_list->fetch())
	{
		...
		{
			$delete_link = ' - <a href="module.php?m=partners&amp;action=categories&amp;delete=' . $categories_list->id . '" ' . 'class="icon delete" ' . 'onclick="return window.confirm(\'' . html::escapeJS(__('m_partners_Confirm_category_deletion')) . '\')">Supprimer</a></p>';
		}
		else
		{
		...
		if ($categories_list->active)
		{
			echo '<a href="module.php?m=partners&amp;action=categories&amp;switch_status=' . $categories_list->id . '" ' . 'class="icon tick">' . __('m_partners_Visible') . '</a>';
		}
		else
		{
			echo '<a href="module.php?m=partners&amp;action=categories&amp;switch_status=' . $categories_list->id . '" ' . 'class="icon cross">' . __('m_partners_Hidden') . '</a>';
		}
		
		echo ' - <a href="module.php?m=partners&amp;action=categories&amp;category_id=' . $categories_list->id . '" ' . 'title="' . __('c_c_action_Edit') . ' ' . html::escapeHTML($categories_list->name) . '" ' . 'class="icon pencil">' . __('c_c_action_Edit') . '</a>';
...
<div id="categories_forms">
	...
	<form action="module.php" method="post" id="edit-category-form">
		<div class="tabered">
			...
			<div id="tab-options">
				<p class="field">
					<label for="edit_category_parent"><?php _e('m_partners_Relative')?></label>

...
if ($dir && ! empty($_POST['newdir']))
{
	try
	{
		...
		$okt['flashMessages']->success(__('Directory has been successfully created.'));
...
if ($dir && ! empty($_FILES['upfile']))
{
	try
	{
		...
		$okt['flashMessages']->success(__('Files have been successfully uploaded.'));
...
if ($dir && ! empty($_POST['rmyes']) && ! empty($_POST['remove']))
{
	...
	try
	{
		...
		$okt['flashMessages']->success(__('File has been successfully removed.'));
...
if ($dir && $okt['visitor']->is_superadmin && ! empty($_POST['rebuild']))
{
	try
	{
		...
		$okt['flashMessages']->success(__('Directory has been successfully rebuilt.'));
...
if ($dir && ! empty($_GET['remove']))
{
	...
	echo '<form action="' . html::escapeURL($page_url) . '" method="post">' . '<p>' . sprintf(__('Are you sure you want to remove %s?'), html::escapeHTML($_GET['remove'])) . '</p>' . '<p><input type="submit" value="' . __('c_c_action_cancel') . '" /> ' . ' &nbsp; <input type="submit" name="rmyes" value="' . __('c_c_yes') . '" />' . form::hidden('d', $d) . Page::formtoken() . form::hidden('remove', html::escapeHTML($_GET['remove'])) . '</p>' . '</form>';
...
else
{
	...
	echo '<form action="module.php" method="get">' . '<p><label>' . __('Sort files:') . ' ' . form::select('file_sort', $sort_combo, $file_sort) . '</label>' . form::hidden(array(
		'm'
	), 'media_manager') . form::hidden(array(
		'popup'
	), $popup) . '<input type="submit" value="' . __('ok') . '" /></p>' . '</form>';
...
if ($core_media_writable)
{
	...
	echo '<div class="col"><h3 id="add-file">' . __('Add files') . '</h3>' . '<form id="media-upload" action="' . html::escapeURL($page_url) . '" method="post" enctype="multipart/form-data">' . 

	'<div>' . form::hidden(array(
		'MAX_FILE_SIZE'
	), Utilities::getMaxUploadSizeNotice()) . Page::formtoken() . '</div>' . 

	'<fieldset id="add-file-f">' . 

	'<p class="field"><label for="upfile">' . __('Choose a file:') . '</label>' . '<input type="file" name="upfile" id="upfile" size="20" /></p>' . 

	'<p class="note">' . Utilities::getMaxUploadSizeNotice() . '</p>' . 

	'<p class="field"><label for="upfiletitle">' . __('Title:') . '</label>' . form::text('upfiletitle', 35, 255) . '</p>' . 

	'<p class="field"><label>' . form::checkbox(array(
		'upfilepriv'
	), 1) . ' ' . __('Private') . '</label></p>' . 

	'<p><input type="submit" value="' . __('c_c_action_send') . '" />' . form::hidden(array(
		'd'
	), $d) . '</p>' . 

	'</fieldset>' . '</form>' . 

	'<p class="note">' . __('Please take care to publish media that you own and that are not protected by copyright.') . '</p>' . '</div>';
	
	echo '<div class="col"><h3 id="new-dir">' . __('New directory') . '</h3>' . '<form class="clear" action="' . html::escapeURL($page_url) . '" method="post">' . '<fieldset id="new-dir-f">' . Page::formtoken() . 

	'<p class="field"><label for="newdir">' . __('Directory Name:') . '</label>' . form::text('newdir', 35, 255) . '</p>' . 

...
if ($file && ! empty($_FILES['upfile']) && $file->editable && $core_media_writable)
{
	try
	{
		...
		$okt['flashMessages']->success(__('File has been successfully updated.'));
...
if (! empty($_POST['thumbs']) && $file->media_type == 'image' && $file->editable && $core_media_writable)
{
	try
	{
		...
		$okt['flashMessages']->success(__('Thumbnails have been successfully updated.'));
...
if (! empty($_POST['unzip']) && $file->type == 'application/zip' && $file->editable && $core_media_writable)
{
	try
	{
		...
		$okt['flashMessages']->success(__('Zip file has been successfully extracted.'));
...
echo '<h3><a href="' . html::escapeURL($media_page_url) . '">' . __('Media manager') . '</a>' . ' / ' . $okt->media->breadCrumb(html::escapeURL($media_page_url) . '&amp;d=%s') . $file->basename . '</h3>';

echo '<div id="tabered">' . '<ul>' . ($popup ? '<li><a href="#media-insert"><span>' . __('Insert media item') . '</span></a></li>' : '') . '<li><a href="#media-details-tab"><span>' . __('Media details') . '</span></a></li>' . '</ul>';
...
*/
	...
	echo '<p><a id="media-insert-cancel" class="button" href="#">' . __('c_c_action_Cancel') . '</a> - ' . '<strong><a id="media-insert-ok" class="button" href="#">' . __('c_c_action_Insert') . '</a></strong>' . form::hidden(array(
		'type'
	), html::escapeHTML($media_type)) . form::hidden(array(
		'title'
	), html::escapeHTML($file->media_title)) . form::hidden(array(
		'description'
	), html::escapeHTML($media_desc)) . form::hidden(array(
		'url'
	), $file->file_url) . '</p>';
...
echo '<div id="media-details-tab">' . '<p id="media-icon"><img src="' . $file->media_icon . '" alt="" /></p>';
...
if ($file->media_image)
{
	...
	foreach (array_reverse($file->media_thumb) as $s => $v)
	{
		...
		printf($strong_link, '<a href="' . html::escapeURL($page_url) . '&amp;id=' . $id . '&amp;size=' . $s . '">' . __($okt->media->thumb_sizes[$s][2]) . '</a> | ');
	}
...
echo '<h3>' . __('Media details') . '</h3>' . '<ul>' . '<li><strong>' . __('File owner:') . '</strong> ' . $file->media_user . '</li>' . '<li><strong>' . __('File type:') . '</strong> ' . $file->type . '</li>' . '<li><strong>' . __('File size:') . '</strong> ' . Utilities::l10nFileSize($file->size) . '</li>' . '<li><strong>' . __('File URL:') . '</strong> <a href="' . $file->file_url . '">' . $file->file_url . '</a></li>' . '</ul>';
...
if ($file->editable && $core_media_writable)
{
	if ($file->media_type == 'image')
	{
		echo '<form class="clear" action="' . html::escapeURL($page_url) . '" method="post">' . '<fieldset><legend>' . __('Update thumbnails') . '</legend>' . '<p>' . __('This will create or update thumbnails for this image.') . '</p>' . '<p><input type="submit" name="thumbs" value="' . __('update thumbnails') . '" />' . form::hidden(array(
			'id'
		), $id) . Page::formtoken() . '</p>' . '</fieldset></form>';
	}
	...
	if ($file->type == 'application/zip')
	{
		...
		echo '<form class="clear" id="file-unzip" action="' . html::escapeURL($page_url) . '" method="post">' . '<fieldset><legend>' . __('Extract archive') . '</legend>' . '<ul>' . '<li><strong>' . __('Extract in a new directory') . '</strong> : ' . __('This will extract archive in a new directory that should not exists yet.') . '</li>' . '<li><strong>' . __('Extract in current directory') . '</strong> : ' . __('This will extract archive in current directory and will overwrite existing files or directory.') . '</li>' . '</ul>' . '<p><label class="classic">' . __('Extract mode:') . ' ' . form::select('inflate_mode', $inflate_combo, 'new') . '</label> ' . '<input type="submit" name="unzip" value="' . __('c_c_action_extract') . '" />' . form::hidden(array(
			'id'
		), $id) . Page::formtoken() . '</p>' . '</fieldset></form>';
	}
	
	echo '<form class="clear" action="' . html::escapeURL($page_url) . '" method="post">' . '<fieldset><legend>' . __('Change media properties') . '</legend>' . 

	'<p class="field"><label>' . __('File name:') . '</label>' . form::text('media_file', 30, 255, html::escapeHTML($file->basename)) . '</p>' . 

	'<p class="field"><label>' . __('File title:') . '</label>' . form::text('media_title', 30, 255, html::escapeHTML($file->media_title)) . '</p>' . 

	'<p class="field"><label>' . __('File date:') . '</label>' . form::text('media_dt', 16, 16, html::escapeHTML($file->media_dtstr)) . '</p>' . '<p class="field"><label class="classic">' . form::checkbox('media_private', 1, $file->media_priv) . ' ' . __('Private') . '</label></p>' . 

	'<p class="field"><label>' . __('New directory:') . '</label>' . form::select('media_path', $dirs_combo, dirname($file->relname)) . '</p>' . 

	'<p><input type="submit" value="' . __('c_c_action_save') . '" />' . form::hidden(array(
		'id'
	), $id) . Page::formtoken() . '</p>' . '</fieldset></form>';
	
	echo '<form class="clear" action="' . html::escapeURL($page_url) . '" method="post" enctype="multipart/form-data">' . '<fieldset><legend>' . __('Change file') . '</legend>' . '<div>' . form::hidden(array(
		'MAX_FILE_SIZE'
	), Utilities::getMaxUploadSizeNotice()) . '</div>' . '<p class="field"><label for="upfile">' . __('Choose a file:') . '</label>' . '<input type="file" id="upfile" name="upfile" size="35" /></p>' . '<p class="note">' . Utilities::getMaxUploadSizeNotice() . '</p>' . '<p><input type="submit" value="' . __('c_c_action_send') . '" />' . form::hidden(array(
		'id'
	), $id) . Page::formtoken() . '</p>' . '</fieldset></form>';
}

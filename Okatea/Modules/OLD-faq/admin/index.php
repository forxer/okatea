...
$pager = new Pager($okt, $okt->faq->filters->params->page, $num_filtered_questions, $okt->faq->filters->params->nb_per_page);
...
<form action="module.php" method="get" id="filters-form">
	<fieldset>
		...
		<input type="submit"
				name="<?php echo $okt->faq->filters->getFilterSubmitName() ?>"
				value="<?php _e('c_c_action_display')?>" /> <a
				href="module.php?m=faq&amp;action=index&amp;init_filters=1"><?php _e('c_c_reset_filters')?></a>
		</p>
	</fieldset>
</form>
...
$count_line = 0;
	...
	<tr>
			<th scope="row" class="<?php echo $td_class ?> fake-td"><a
				href="module.php?m=faq&amp;action=edit&amp;questions_id=<?php echo $list->id ?>"><?php echo html::escapeHTML($list->title) ?></a></th>
		<?php if ($okt->faq->config->enable_categories) : ?>
		...
		<td class="<?php echo $td_class ?> small">
				<ul class="actions">
					<li><a
						href="module.php?m=faq&amp;action=index&amp;switch_status=<?php echo $list->id ?>"
						title="<?php printf(__('m_faq_switch_visibility_%s'), html::escapeHTML($list->title)) ?>"
						<?php if ($list->active == 0) : ?> class="icon cross"><?php _e('c_c_status_Offline')?></a>
					<?php else : ?>
					...
					<li><a
						href="module.php?m=faq&amp;action=edit&amp;questions_id=<?php echo $list->id ?>"

...
if (! empty($_REQUEST['questions_id']))
{
	...
	$question_infos = $okt->faq->getQuestions(array(
		'id' => $questions_id,
		'active' => 2
	));
...
<form action="module.php" method="post" enctype="multipart/form-data">
	<div id="tabered">
		...
		<div id="tab_question">
			...
			<p class="field" lang="<?php echo $aLanguage['code'] ?>">
				<label for="p_content_<?php echo $aLanguage['code'] ?>"><?php _e('m_faq_content')?> <span
					class="lang-switcher-buttons"></span></label>
		...
		<div id="tab_options">
			<div class="two-cols">
				<p class="field col">
					<label><?php echo form::checkbox('p_active', 1, $p_active) ?> <?php _e('c_c_status_online') ?></label>
				</p>

				<?php if ($okt->faq->config->enable_categories) : ?>
				<p class="field col">
					<label for="p_cat_id"><?php _e('m_faq_section') ?></label>
		...
		<div id="tab_images">
			...
			<?php for ($i=1; $i<=$okt->faq->config->images['number']; $i++) : ?>
				<div class="col">
					...
					<p class="field">
						<label for="p_images_<?php echo $i ?>"><?php printf(__('m_faq_images_%s'),$i) ?></label>
...
# il y a une image ?
				...
				:
					...
					?>
						...
						<p class="field">
						<label for="p_images_alt_<?php echo $i ?>"><?php printf(__('m_faq_alternative_text_image_%s'), $i) ?></label>
						<?php echo form::text('p_images_alt_'.$i,40,255,html::escapeHTML($cur_image_alt)) ?></p>

					<p>
						<a href="<?php echo $post_images[$i]['img_url']?>"
							rel="questions_images" class="modal"><img
					...
					<p>
						<a
							href="module.php?m=faq&amp;action=edit&amp;questions_id=<?php
					echo $questions_id?>&amp;delete_image=<?php echo $i ?>"
							class="icon delete"><?php _e('m_faq_delete_image')?></a>
					</p>
		...
		<?php if ($okt->faq->config->files['enable']) : ?>
		<div id="tab_files">
			<h3><?php _e('m_faq_attached_files')?> <span
			...
			<div class="two-cols" lang="<?php echo $aLanguage['code'] ?>">
				...
				<div class="col">
					...
					<p class="field">
						<label
							for="p_files_<?php echo $aLanguage['code'] ?>_<?php echo $i ?>">Fichier <?php echo $i+1 ?></label>
				...
				<p>
						<img
							src="<?php
						echo $okt['config']->app_url . 'oktCommon/img/media/' . $files_infos[$aLanguage['code']][$i]['type'] . '.png'?>"
				...
				- <a
							href="module.php?m=faq&amp;action=edit&amp;questions_id=<?php echo $questions_id ?>&amp;delfile=<?php echo $files_infos[$aLanguage['code']][$i]['filename'] ?>"><?php _e('c_c_action_delete')?></a>
					</p>
				<?php endif; ?>
				</div>
				<?php endfor; ?>
		...
		<div id="tab_seo">
			...
			<p class="field" lang="<?php echo $aLanguage['code'] ?>">
				<label for="p_title_seo_<?php echo $aLanguage['code'] ?>"><?php _e('c_c_seo_title_seo')?> <span
					class="lang-switcher-buttons"></span></label>
			...
			<div class="lockable">
				<p class="field" lang="<?php echo $aLanguage['code'] ?>">
					<label for="p_slug_<?php echo $aLanguage['code'] ?>">URL <span
						class="lang-switcher-buttons"></span></label>
	...
	<input type="submit"
			value="<?php echo !empty($questions_id) ? _e('c_c_action_edit') : _e('c_c_action_add'); ?>" />
	</p>
</form>

...
class module_faq extends Module
{
	...
	protected function prepend_admin()
	{
		...
		if ($this->okt->page->display_menu)
		{
			...
			$this->okt->page->mainMenu->add($this->getName(), 'module.php?m=faq', $this->bCurrentlyInUse, 10, $this->okt->checkPerm('faq'), null, $this->okt->page->faqSubMenu, $this->okt->options->public_url . '/modules/' . $this->id() . '/module_icon.png');
	...
	public function getQuestions($params = array(), $count_only = false)
	{
		...
		if ($count_only)
		{
			$query = 'SELECT COUNT(p.id) AS num_questions ' . 'FROM ' . $this->t_faq . ' AS p ' . 'LEFT JOIN ' . $this->t_faq_locales . ' AS pl ON pl.faq_id=p.id ' . 'LEFT JOIN ' . $this->t_faq_cat . ' AS c ON c.id=p.cat_id ' . 'LEFT JOIN ' . $this->t_faq_cat_locales . ' AS cl ON cl.cat_id=p.cat_id ' . 'WHERE 1 ' . $reqPlus;
		}
		else
		{
			$query = 'SELECT p.id, p.cat_id, cl.title AS category, p.active, p.images, p.files, ' . 'pl.title, pl.title_tag, pl.title_seo, pl.slug, pl.content, pl.meta_description, pl.meta_keywords ' . 'FROM ' . $this->t_faq . ' AS p ' . 'LEFT JOIN ' . $this->t_faq_locales . ' AS pl ON pl.faq_id=p.id ' . 'LEFT JOIN ' . $this->t_faq_cat . ' AS c ON c.id=p.cat_id ' . 'LEFT JOIN ' . $this->t_faq_cat_locales . ' AS cl ON cl.cat_id=p.cat_id ' . 'WHERE 1 ' . $reqPlus;
	...
	{
		$query = 'SELECT language, title, title_tag, title_seo, slug, content, meta_description, meta_keywords ' . 'FROM ' . $this->t_faq_locales . ' ' . 'WHERE faq_id=' . (integer) $question_id;
	...
	public function getCategories($params = array())
	{
		...
		$query = 'SELECT c.id, c.active, c.ord, cl.title ' . 'FROM ' . $this->t_faq_cat . ' AS c ' . 'LEFT JOIN ' . $this->t_faq_cat_locales . ' AS cl ON cl.cat_id=c.id ' . 'WHERE 1 ' . $reqPlus;
	...
	{
		$query = 'SELECT language, title ' . 'FROM ' . $this->t_faq_cat_locales . ' ' . 'WHERE cat_id=' . (integer) $iCategoryId;
	...
	public function regenMinImages()
	{
		...
		while ($rsQuestions->fetch())
		{
			...
			foreach ($aImages as $key => $image)
			{
				...
				$aImagesList[$key] = array_merge($aImages[$key], $this->getImageUpload()->buildImageInfos($rsQuestions->id, $image['img_name']));
			}
	...
	public function indexAllQuestions()
	{
		...
		while ($rsQuestions->fetch())
		{
			...
			{
				$words = $rsQuestionLocales->title . ' ' . $rsQuestionLocales->subtitle . ' ' . $rsQuestionLocales->content . ' ';
				
				$words = implode(' ', Modifiers::splitWords($words));
				
				$query = 'UPDATE ' . $this->t_faq_locales . ' SET ' . 'words=\'' . $this->db->escapeStr($words) . '\' ' . 'WHERE faq_id=' . (integer) $rsQuestions->id . ' AND language=\'' . $this->db->escapeStr($rsQuestionLocales->language) . '\' ';
	...
	protected function buildQuestionSlug($title, $url, $question_id, $lang_code)
	{
		...
		$query = 'SELECT slug FROM ' . $this->t_faq_locales . ' ' . 'WHERE slug=\'' . $this->db->escapeStr($url) . '\' ' . 'AND faq_id <> ' . (integer) $question_id . ' ' . 'AND language=\'' . $this->db->escapeStr($lang_code) . '\' ' . 'ORDER BY slug DESC';

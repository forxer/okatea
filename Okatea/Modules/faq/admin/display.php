	...
	<div id="tabered">
		...
		<div id="tab_public">
			...
			<fieldset>
				...
				<p class="field col"><label for="p_public_default_nb_per_page"><?php _e('m_faq_public_number_questions')?></label>
				<?php echo form::text('p_public_default_nb_per_page', 3, 3, $okt->faq->config->public_default_nb_per_page) ?></p>

				<p class="field"><label for="p_public_default_order_by"><?php _e('m_faq_public_order_display') ?></label>
				<?php echo form::select('p_public_default_order_by',$field_choice_order_by,$okt->faq->config->public_default_order_by) ?></p>

				<p class="field"><label for="p_public_default_order_direction"><?php _e('m_faq_public_display_direction') ?></label>
			...
			<fieldset>
				...
				<p class="field"><label for="p_public_truncat_char"><?php _e('m_faq_public_trucate_number')?></label>
		...
		<div id="tab_admin">
			...
			<fieldset>
				...
				<p class="field col"><label for="p_admin_default_nb_per_page"><?php _e('m_faq_admin_number_questions') ?></label>
				<?php echo form::text('p_admin_default_nb_per_page', 3, 3, $okt->faq->config->admin_default_nb_per_page) ?></p>

				<p class="field"><label for="p_admin_default_order_by"><?php _e('m_faq_admin_order_display') ?></label>
				<?php echo form::select('p_admin_default_order_by',$field_choice_order_by,$okt->faq->config->admin_default_order_by) ?></p>

				<p class="field"><label for="p_admin_default_order_direction"><?php _e('m_faq_admin_display_direction') ?></label>
		...
		<div id="tab_images">
			...
			<fieldset>
				...
				<?php if ($okt->page->hasLbl()) : ?>
					<p class="field"><label for="p_lightbox_type"><?php _e('m_faq_choose_display')?></label>
					...
					<p><?php _e('m_faq_currently_used')?> <em><?php $aChoices = array_merge(array(''=>__('c_c_none_f')),$okt->page->getLblList());
					echo $aChoices[$okt->faq->config->lightbox_type] ?></em></p>
				<?php else : ?>
					<p><span class="icon error"></span><?php _e('m_faq_no_interface_images')?>
					<?php echo form::hidden('p_lightbox_type',0); ?></p>
				<?php endif;?>

				<p class="modal-box">
					<a class="modal" rel="test_images" href="<?php echo $okt->options->public_url ?>/img/sample/chutes_la_nuit.jpg">
					<img width="60" height="60" alt="" src="<?php echo $okt->options->public_url ?>/img/sample/sq-chutes_la_nuit.jpg"/></a>

					<a class="modal" rel="test_images" href="<?php echo $okt->options->public_url ?>/img/sample/les_chutes.jpg">
					<img width="60" height="60" alt="" src="<?php echo $okt->options->public_url ?>/img/sample/sq-les_chutes.jpg"/></a>

					<a class="modal" rel="test_images" href="<?php echo $okt->options->public_url ?>/img/sample/chutes.jpg">

...
class module_accessible_captcha extends Module
{
	...
	protected function prepend()
	{
		...
		$this->okt->page->addCaptcha('accessible_captcha', __('Accessible Captcha'), array(
			...
			'publicModuleContactControllerStart' => array(
				'module_accessible_captcha',
				'publicControllerStart'
			),
			'publicModuleContactControllerFormCheckValues' => array(
				'module_accessible_captcha',
				'publicControllerFormCheckValues'
			),
			'publicModuleContactJsValidateRules' => array(
				'module_accessible_captcha',
				'publicJsValidateRules'
			),
			'publicModuleContactTplFormBottom' => array(
				'module_accessible_captcha',
				'publicTplFormBottom'
			),
			
			# behaviors estimate
			'publicModuleEstimateControllerStart' => array(
				'module_accessible_captcha',
				'publicControllerStart'
			),
			'publicModuleEstimateControllerFormCheckValues' => array(
				'module_accessible_captcha',
				'publicControllerFormCheckValues'
			),
			'publicModuleEstimateJsValidateRules' => array(
				'module_accessible_captcha',
				'publicJsValidateRules'
			),
			'publicModuleEstimateTplFormBottom' => array(
				'module_accessible_captcha',
				'publicTplFormBottom'
			),
			
			# behaviors livre d'or
			'publicModuleGuestbookControllerStart' => array(
				'module_accessible_captcha',
				'publicControllerStart'
			),
			'publicModuleGuestbookControllerFormCheckValues' => array(
				'module_accessible_captcha',
				'publicControllerFormCheckValues'
			),
			'publicModuleGuestbookJsValidateRules' => array(
				'module_accessible_captcha',
				'publicJsValidateRules'
			),
			'publicModuleGuestbookTplFormBottom' => array(
				'module_accessible_captcha',
				'publicTplFormBottom'
			)
		));
	...
	protected function prepend_admin()
	{
		...
		{
			$this->okt->page->configSubMenu->add(__('Accessible Captcha'), 'module.php?m=accessible_captcha&amp;action=index', $this->bCurrentlyInUse && (! $this->okt->page->action || $this->okt->page->action === 'index'), 30, $this->okt->checkPerm('accessible_captcha_config'), null);
		}
	}
	 ...
	 *
	 * @param
	 *        	string question		La question encodée
	 * @param
	 *        	string	answer			La réponse
	 * @return boolean
	 */
	 ...
	 * @param
	 *        	string	language_code	Le code de la langue
	 * @return boolean
	 */
	 ...
	 *
	 * @param
	 *        	integer	id				Identifiant de la question
	 * @param
	 *        	string	question		La question
	 * @param
	 *        	string	reponse			La réponse
	...
	public static function publicTplFormBottom($okt, $sCaptchaId)
	{
		if ($sCaptchaId == 'accessible_captcha')
		{
			echo '<fieldset id="accessible_captcha">' . '<legend>' . __('SPAM prevention') . '</legend>' . '<p>' . __('In order to validate the form please answer the following question, this allows to ensure us that you are not a spam robot. Thank you for your comprehension.') . '</p>' . '<p class="field"><label for="captcha">' . $okt->accessible_captcha->getQuestion() . '</label>' . '<input name="captcha" id="captcha" type="text" class="text" size="50" maxlength="255" />' . '<input name="captcha_q" value="' . $okt->accessible_captcha->getEncQuestion() . '" type="hidden" /></p>' . '</fieldset>';
		}
	}
}

...
if (!empty($_GET['delete']))
{
	...
	{
		$okt['flashMessages']->success(__('Catégorie supprimée.'));
...
if (!empty($_POST['add_category']))
{
	...
	if ($okt->error->isEmpty())
	{
		...
		{
			$okt['flashMessages']->success(__('La catégorie a été ajoutée.'));
...
if (!empty($_POST['edit_category']) && $category_id)
{
	...
	if ($okt->error->isEmpty())
	{
		...
		{
			$okt['flashMessages']->success(__('La catégorie a été  mise à jour.'));
...
if (!empty($_POST['ordered']) && !empty($order))
{
	...
	$okt['flashMessages']->success(__('L’ordre des catégories a été mis à jour.'));
...
$okt->page->js->addReady('
	$("#sortable").sortable({
		...
		update: function(event, ui) {
			...
			$.ajax({
				data: result,
				url: "' . $okt['modules_url'] . '/catalog/service_ordering_categories.php",
...
$okt->page->css->addCSS('
	#ajaxloader {
		float: right;
		display: none;
		background: transparent url(' . $okt['public_url'] . '/img/ajax-loader/indicator.gif) no-repeat 0 0;
		width: 16px;
		height: 16px;
	}
');
	...
	<form action="module.php" method="post" id="add-category-form">
		...
		<p class="field">
			<label for="add_category_name"
				title="<?php _e('c_c_required_field') ?>" class="required">Nom</label>
		<?php echo form::text('add_category_name', 50, 255, html::escapeHTML($add_category_name)) ?></p>

		<p class="field">
			<label for="add_category_parent">Parent</label> <select
				id="add_category_parent" name="add_category_parent">
...
<?php
	...
	while ($categories_list->fetch())
	{
		...
		{
			$delete_link = ' - <a href="module.php?m=catalog&amp;action=categories&amp;delete=' . $categories_list->id . '" ' . 'class="icon delete" ' . 'onclick="return window.confirm(\'' . html::escapeJS('Etes-vous sûr de vouloir supprimer cette catégorie ? Cette action est irréversible.') . '\')">Supprimer</a></p>';
		}
		else
		{
		...
		if ($categories_list->active)
		{
			echo '<a href="module.php?m=catalog&amp;action=categories&amp;switch_status=' . $categories_list->id . '" ' . 'class="icon tick">visible</a>';
		}
		else
		{
			echo '<a href="module.php?m=catalog&amp;action=categories&amp;switch_status=' . $categories_list->id . '" ' . 'class="icon cross">masquée</a>';
		}
		
		echo ' - <a href="module.php?m=catalog&amp;action=categories&amp;category_id=' . $categories_list->id . '" ' . 'title="Modifier la catégorie ' . html::escapeHTML($categories_list->name) . '" ' . 'class="icon pencil">Modifier</a>';
...
<div id="categories_forms">
	...
	<form action="module.php" method="post" id="edit-category-form">
		...
		<p class="field">
			<label for="edit_category_parent">Parent</label>
		<?php echo form::select('edit_category_parent',$edit_allowed_parents,$edit_category_parent) ?></p>

		<p class="field">
			<label><?php echo form::checkbox('edit_category_active', 1, $edit_category_active) ?> Active</label>
		</p>
...
$i = 1;
		...
		:
			?>
		<li id="ord_<?php echo $siblings->id; ?>" class="ui-state-default"><label
				for="order_<?php echo $siblings->id ?>"> <span
		...
		<?php echo Page::formtoken(); ?>
		<input type="submit" value="enregistrer l’ordre" id="save_order" />
		</p>
	</form>
	<?php endif; ?>
</div>

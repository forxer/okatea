	...
	if ($okt->catalog->checkProdData($product_data))
	{
		...
		if (! empty($product_id))
		{
			...
			if ($okt->catalog->updProd($product_id, $cursor) !== false)
			{
				...
				$okt['flashMessages']->success(__('Le produit a été mis à jour.'));
		...
		else
		{
			...
			if (($product_id = $okt->catalog->addProd($cursor)) !== false)
			{
				...
				$okt['flashMessages']->success(__('Le produit a été ajouté.'));
...
{
	$categories_list = $okt->catalog->getCategories(array(
		'active' => 2
	));
}
...
<p>
	Produit ajouté le <em><?php echo dt::dt2str(__('%A, %B %d, %Y, %H:%M'),$product_data['created_at']) ?></em>.
	<?php if ($product_data['updated_at'] > $product_data['created_at']) : ?>
	<span class="note">Dernière modification le <em><?php echo dt::dt2str(__('%A, %B %d, %Y, %H:%M'),$product_data['updated_at']) ?></em>.
	</span>
	<?php endif; ?>
</p>
<?php endif; ?>

<form id="catalog-form" action="module.php" method="post"
	...
	<div id="tabered">
		...
		<div id="tab-content">
			...
			<?php if ($okt->catalog->config->fields['subtitle']) : ?>
				...
				<label for="p_subtitle"
					<?php if ($okt->catalog->config->fields['subtitle'] == 2) : ?>
					title="<?php _e('c_c_required_field') ?>" class="required"
					<?php endif; ?>>Sous-titre</label>
			...
			<?php if ($okt->catalog->config->fields['content_short']) : ?>
				<p class="col field">
				<label for="p_content_short"
					<?php if ($okt->catalog->config->fields['content_short'] == 2) : ?>
					title="<?php _e('c_c_required_field') ?>" class="required"
					<?php endif; ?>>Description rapide</label>
			<?php echo form::textarea('p_content_short', 57, 5, html::escapeHTML($product_data['content_short'])) ?></p>
			<?php endif; ?>

			<p class="field">
				<label for="p_content" title="<?php _e('c_c_required_field') ?>"
					class="required">Description détaillée</label>
			<?php echo form::textarea('p_content', 57, 10, html::escapeHTML($product_data['content'])) ?></p>

			<?php if ($okt->catalog->config->fields['price']) : ?>
				<p class="col field">
				<label for="p_price"
					<?php if ($okt->catalog->config->fields['price'] == 2) : ?>
					title="<?php _e('c_c_required_field') ?>" class="required"
					<?php endif; ?>>Prix (en euros)</label>
		...
		<div id="tab-options">
			...
			<div class="three-cols">
				...
				<p class="field col">
					<label for="p_category_id">Catégorie</label> <select
						id="p_category_id" name="p_category_id">
						<option value="0">Premier niveau</option>
					<?php
					while ($categories_list->fetch())
					{
						echo '<option value="' . $categories_list->id . '"' . ($categories_list->id == $product_data['category_id'] ? ' selected="selected"' : '') . '>' . str_repeat('&nbsp;&nbsp;', $categories_list->level) . '&bull; ' . html::escapeHTML($categories_list->name) . '</option>';
					}
					?>
				</select>
				</p>
			...
			<?php if (!empty($product_id)) : ?>
				<p class="field col">
					<label for="p_visibility">État</label>
			...
			<div class="three-cols">
				<?php if ($okt->catalog->config->fields['promo'] == 1) : ?>
					<p class="col field">
					<label><?php echo form::checkbox('p_promo', 1, $product_data['promo'])?>
					<?php _e('m_catalog_action_promo') ?></label>
				</p>

				<?php elseif ($okt->catalog->config->fields['promo'] == 2) : ?>
					<p class="col field">
					<label for="p_promo_start"><?php _e('m_catalog_tab_status_start_promo') ?></label>
					<?php echo form::text('p_promo_start', 20, 255, ($product_data['promo_start'] ? dt::dt2str('%d-%m-%Y',$product_data['promo_start']) : ''), 'datepicker') ?></p>

				<p class="col field">
					<label for="p_promo_end"><?php _e('m_catalog_tab_status_end_promo') ?></label>
					<?php echo form::text('p_promo_end', 20, 255, ($product_data['promo_end'] ? dt::dt2str('%d-%m-%Y',$product_data['promo_end']) : ''), 'datepicker') ?></p>
				<?php endif; ?>

					<p class="col field">
					<label for="p_price_promo"><?php _e('m_catalog_tab_status_promo_price') ?></label>
			...
			<div class="three-cols">
				...
				<p class="col field">
					<label><?php echo form::checkbox('p_nouvo', 1, $product_data['nouvo'])?>
				<?php _e('m_catalog_action_nouvo') ?></label>
				</p>

				<?php elseif ($okt->catalog->config->fields['nouvo'] == 2) : ?>
				<p class="col field">
					<label for="p_nouvo_start"><?php _e('m_catalog_tab_status_start_novelty') ?></label>
				<?php echo form::text('p_nouvo_start', 20, 255, ($product_data['nouvo_start'] ? dt::dt2str('%d-%m-%Y',$product_data['nouvo_start']) : ''), 'datepicker') ?></p>

				<p class="col field">
					<label for="p_nouvo_end"><?php _e('m_catalog_tab_status_end_novelty') ?></label>
			...
			<div class="three-cols">
				...
				<p class="col field">
					<label><?php echo form::checkbox('p_favo', 1, $product_data['favo'])?>
				<?php _e('m_catalog_action_favo') ?></label>
				</p>

				<?php elseif ($okt->catalog->config->fields['favo'] == 2) : ?>
				<p class="col field">
					<label for="p_favo_start"><?php _e('m_catalog_tab_status_start_favo') ?></label>
				<?php echo form::text('p_favo_start', 20, 255, ($product_data['favo_start'] ? dt::dt2str('%d-%m-%Y',$product_data['favo_start']) : ''), 'datepicker') ?></p>

				<p class="col field">
					<label for="p_favo_end"><?php _e('m_catalog_tab_status_end_favo') ?></label>
		...
		<div id="tab_images">
			...
			<?php for ($i=1; $i<=$okt->catalog->config->images['number']; $i++) : ?>
				<div class="col">
					...
					<p class="field">
						<label for="p_images_<?php echo $i ?>">Image <?php echo $i ?></label>
...
# il y a une image ?
				...
				:
					...
					?>
						...
						<p class="field">
						<label for="p_images_alt_<?php echo $i ?>">Texte alternatif de l’image <?php echo $i ?></label>
						<?php echo form::text('p_images_alt_'.$i,40,255,html::escapeHTML($cur_image_alt)) ?></p>

					<p>
						<a href="<?php echo $post_images[$i]['img_url']?>"
							title="<?php echo html::escapeHTML($product_data['title']) ?>, image <?php echo $i ?>"
							rel="catalog_images" class="modal"><img
							src="<?php echo $cur_image_url ?>" <?php echo $cur_image_attr?>
							alt="" /></a>
					</p>
						...
						<p>
						<a
							href="module.php?m=catalog&amp;action=edit&amp;product_id=<?php
						echo $product_id?>&amp;delete_image=<?php echo $i ?>"
							class="icon delete">supprimer cette image</a>
					</p>
						<?php endif; ?>
		...
		<div id="tab_files">
			...
			<div class="col">
					<p class="field">
						<label for="p_files_<?php echo $i ?>">Fichier <?php echo $i ?></label>
...
# il y a un fichier ?
				...
				:
					...
					<p class="field">
						<label for="p_files_title_<?php echo $i ?>">Titre fichier <?php echo $i ?></label>
					<?php echo form::text('p_files_title_'.$i,40,255,html::escapeHTML($post_files[$i]['title'])) ?></p>

					<p>
						<a href="<?php echo $post_files[$i]['url'] ?>"><img
							src="<?php echo $okt['public_url'].'/img/media/'.$post_files[$i]['type'].'.png' ?>"
							alt="<?php echo html::escapeHTML($post_files[$i]['title']) ?>" /></a>
					...
					<?php if ($can_edit_product) : ?>
						...
						echo $product_id?>&amp;delete_file=<?php echo $i ?>"
							...
							class="icon delete">supprimer ce fichier</a>
					</p>
					<?php endif; ?>
		...
		<div id="tab-seo">
			...
			<p class="field">
				<label for="p_title_tag">Élément title du produit</label>
	...
	<input type="submit"
			value="<?php echo !empty($product_id) ? 'modifier' : 'ajouter'; ?>" />
	</p>
</form>

	...
	<div id="tabered">
		...
		<div id="tab_general">
			...
			<fieldset>
				...
				<p class="field">
					<label><?php echo form::checkbox('p_categories_enable',1,$okt->catalog->config->categories_enable)?>
				Activer les categories</label>
				</p>

				<p class="field">
					<label><?php echo form::checkbox('p_seo_enable',1,$okt->catalog->config->seo_enable)?>
				<?php _e('c_c_enable_seo_help') ?></label>
				</p>

				<p class="field">
					<label><?php echo form::checkbox('p_enable_filters',1,$okt->catalog->config->enable_filters)?>
				Afficher les filtres sur la partie publique</label>
				</p>

			<?php if ($okt->page->hasRte()) : ?>
				<p class="field">
					<label for="p_rte_enable">Éditeur de texte riche</label>
		...
		<div id="tab_fields">
			...
			<fieldset class="three-cols">
				...
				<p class="field col">
					<label for="p_chp_subtitle"><?php _e('m_catalog_action_subtitle') ?></label>
					<?php echo form::select('p_chp_subtitle', $field_choice, $p_chp_subtitle) ?></p>

				<p class="field col">
					<label for="p_chp_content_short"><?php _e('m_catalog_action_content_short') ?></label>
					<?php echo form::select('p_chp_content_short', $field_choice, $p_chp_content_short) ?></p>

				<p class="field col">
					<label for="p_chp_price"><?php _e('m_catalog_action_price') ?></label>
			...
			<fieldset class="three-cols">
				...
				<p class="field col">
					<label for="p_chp_promo"><?php _e('m_catalog_action_promo') ?></label>
					<?php echo form::select('p_chp_promo', $field_choice_pnf, $p_chp_promo) ?></p>

				<p class="field col">
					<label for="p_chp_nouvo"><?php _e('m_catalog_action_nouvo') ?></label>
					<?php echo form::select('p_chp_nouvo', $field_choice_pnf, $p_chp_nouvo) ?></p>

				<p class="field col">
					<label for="p_chp_favo"><?php _e('m_catalog_action_favo') ?></label>
		...
		<div id="tab_files">
			...
			<fieldset>
				...
				<p class="field">
					<label><?php echo form::checkbox('p_enable_files',1,$okt->catalog->config->files['enable'])?>
				Activer les fichiers joints</label>
				</p>

				<p class="field">
					<label for="p_number_files">Nombre de fichiers joints</label>
				<?php echo form::text('p_number_files', 10, 255, $okt->catalog->config->files['number']) ?></p>

				<p class="field">
					<label for="p_allowed_exts">Liste des extensions autorisées
		...
		<div id="tab_seo">
			...
			<fieldset>
				...
				<p class="field">
					<label for="p_name"><?php _e('c_c_seo_module_intitle') ?></label>
				<?php echo form::text('p_name', 40, 255, html::escapeHTML($okt->catalog->config->name)) ?></p>

				<p class="field">
					<label for="p_title"><?php _e('c_c_seo_module_title_tag') ?></label>

...
# image
			...
			<p class="modal-box">
				<a href="<?php echo $image['img_url']?>"
					title="<?php echo html::escapeHTML($list->title) ?>" class="modal">
					<img src="<?php echo $image['min_url']?>"
					<?php echo $image['min_attr']?> alt="" />
				</a>
			</p>
...
# catégorie
		...
		<p class="clearer">Ajouté le <?php echo dt::dt2str(__('%Y-%m-%d %H:%M'),$list->created_at) ?>.
			...
			<ul class="actions">
				<li><a
					href="module.php?m=catalog&amp;action=index&amp;switch_status=<?php echo $list->id ?>"
					title="Basculer la visibilité du produit <?php echo html::escapeHTML($list->title) ?>"
					<?php if ($list->visibility == 0) : ?> class="icon cross">masqué
				<?php else : ?>
				class="icon tick">visible
				<?php endif; ?></a></li>

				<li><a
					href="module.php?m=catalog&amp;action=edit&amp;product_id=<?php echo $list->id ?>"
					title="Modifier l'actualité <?php echo html::escapeHTML($list->title) ?>"
					class="icon pencil">Modifier</a></li>

			<?php if ($list->isDeletable()) : ?>
			<li><a
					href="module.php?m=catalog&amp;action=delete&amp;product_id=<?php echo $list->id ?>"
					...
					class="icon delete">Supprimer</a></li>
			<?php endif; ?>
		</ul>
		</div></li>
	<?php endwhile; ?>
...
<table class="common">
	...
	<thead>
		<tr>
			<th scope="col"
				<?php if ($okt->catalog->config->images['enable']) echo ' colspan="2"' ?>>Titre</th>
		...
		<td class="<?php echo $td_class ?> modal-box">
			...
			<a href="<?php echo $image['img_url']?>"
				title="<?php echo html::escapeHTML($list->title) ?>" class="modal">
					...
					<?php echo $image['square_attr']?> alt="" />
			</a>
			<?php endif; ?>
		</td>
		<?php endif; ?>

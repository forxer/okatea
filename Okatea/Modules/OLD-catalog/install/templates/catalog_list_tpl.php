...
<form
	action="<?php echo $view->escape(CatalogHelpers::getCatalogUrl()) ?>"
	method="get"
	id="<?php echo $okt->catalog->filters->getFilterFormId() ?>"
	...
	<fieldset>
		...
		<p class="center">
			...
			<a
				href="<?php echo $view->escape(CatalogHelpers::getCatalogUrl()) ?>?catalog_init_filters=1"
				class="italic"><?php _e('m_catalog_display_filters_init') ?></a>
		</p>
...
<?php 
# début Okatea : si il n’y a PAS de produit à afficher on peut indiquer un message
if ($productsList->isEmpty())
:
	?>

<p>
	<em><?php _e('m_catalog_there_is_no_product') ?></em>
</p>
...
<div id="products_list">
	...
	<?php 
# début Okatea : boucle sur la liste des produits
	...
	<div class="product <?php echo $productsList->odd_even ?>">
		...
		<h2 class="product-title">
			<a href="<?php echo $view->escape($productsList->url) ?>"><?php echo $view->escape($productsList->title) ?></a>
		</h2>
...
# début Okatea : affichage du sous-titre
		...
		:
			...
			<strong><?php echo $view->escape($productsList->subtitle) ?></strong>
		</p>
		...
		:
			?>
				...
				<?php if ($productsList->content_short != ''  && $okt->catalog->config->fields['content_short'] != 0) : ?>
					...
					<p><?php echo $productsList->content_short ?>… <a
					href="<?php echo $view->escape($productsList->url) ?>"
					title="<?php echo $view->escapeHtmlAttr(sprintf(__('m_catalog_more_details_on_%s'),$productsList->title)) ?>"><?php _e('m_catalog_more_details') ?></a>
			</p>

				<?php elseif ($productsList->content != '') : ?>

					<p><?php echo $productsList->content ?>… <a
					href="<?php echo $view->escape($productsList->url) ?>"
					title="<?php echo $view->escapeHtmlAttr(sprintf(__('m_catalog_more_details_on_%s'),$productsList->title)) ?>"><?php _e('m_catalog_more_details') ?></a>
			</p>

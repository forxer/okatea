...
{
	$okt['flashMessages']->error(sprintf(__('m_estimate_estimate_%s_not_exists'), $iEstimateId));
...
if (!empty($_GET['treated']))
{
	try
	{
		...
		$okt['flashMessages']->success(__('m_estimate_details_marked_as_treated'));
...
if (!empty($_GET['untreated']))
{
	try
	{
		...
		$okt['flashMessages']->success(__('m_estimate_details_marked_as_untreated'));

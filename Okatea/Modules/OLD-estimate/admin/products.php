...
if (! empty($_GET['delete_product']))
{
	...
	{
		$okt['flashMessages']->success(__('m_estimate_product_deleted'));
...
$count_line = 0;
	...
	<tr>
			<th class="<?php echo $td_class ?> fake-td" scope="row"><a
				href="module.php?m=estimate&amp;action=product&amp;product_id=<?php echo $rsProducts->id ?>"><?php
		...
		<td class="<?php echo $td_class ?> small">
				...
				<a
						href="module.php?m=estimate&amp;action=products&amp;switch_status=<?php echo $rsProducts->id ?>"
				...
				<?php endif; ?>
					...
					<li><a
						href="module.php?m=estimate&amp;action=product&amp;product_id=<?php echo $rsProducts->id ?>"
						title="<?php printf(__('c_c_action_Edit_%s'), $view->escapeHtmlAttr($rsProducts->title)) ?>"
						class="icon pencil"><?php _e('c_c_action_edit')?></a></li>
					<li><a
						href="module.php?m=estimate&amp;action=products&amp;delete_product=<?php echo $rsProducts->id ?>"

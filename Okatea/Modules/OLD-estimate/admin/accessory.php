...
<form id="accessory-form" action="module.php" method="post">
	<div class="two-cols">
		<p class="field col">
			<label for="p_title" title="<?php _e('c_c_required_field') ?>"
				class="required"><?php _e('m_estimate_accessory_title')?></label>
		<?php echo form::text('p_title', 40, 255, html::escapeHTML($aAccessoryData['title'])) ?></p>

		<p class="field col">
			<label for="p_product_id" title="<?php _e('c_c_required_field') ?>"
				class="required"><?php _e('m_estimate_accessory_product')?></label>
		<?php echo form::select('p_product_id',$aProducts,$aAccessoryData['product_id'])?></p>

		<p class="field col">
			<label><?php echo form::checkbox('p_active', 1, $aAccessoryData['active']) ?> <?php _e('c_c_action_visible')?></label>
		</p>
	</div>
	...
	<input type="submit"
			value="<?php echo (!empty($iAccessoryId) ? __('c_c_action_edit') : __('c_c_action_add')); ?>" />
	</p>
</form>

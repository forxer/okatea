...
if (! empty($_POST['form_sent']))
{
	...
	if (! empty($iProductId))
	{
		...
		if ($okt->estimate->products->updProduct($aProductData) !== false)
		{
			...
			$okt['flashMessages']->success(__('m_estimate_product_modified'));
	...
	else
	{
		if (($iProductId = $okt->estimate->products->addProduct($aProductData)) !== false)
		{
			...
			$okt['flashMessages']->success(__('m_estimate_product_added'));
...
<form id="product-form" action="module.php" method="post"
	enctype="multipart/form-data">

	<div class="two-cols">
		<p class="field col">
			<label for="p_title" title="<?php _e('c_c_required_field') ?>"
				class="required"><?php _e('m_estimate_product_title')?></label>
		<?php echo form::text('p_title', 40, 255, html::escapeHTML($aProductData['title'])) ?></p>

		<p class="field col">
			<label><?php echo form::checkbox('p_active', 1, $aProductData['active']) ?> <?php _e('c_c_action_visible')?></label>
		</p>
	</div>
	...
	<input type="submit"
			value="<?php echo (!empty($iProductId) ? __('c_c_action_edit') : __('c_c_action_add')); ?>" />
	</p>
</form>
...
$count_line = 0;
		...
		:
			...
			<th class="<?php echo $td_class ?> fake-td" scope="row"><a
				href="module.php?m=estimate&amp;action=accessory&amp;accessory_id=<?php echo $rsAccessories->id ?>&amp;product_id=<?php echo $iProductId ?>"><?php
			...
			<td class="<?php echo $td_class ?> small">
				<ul class="actions">
					<li>
						...
						<a
						href="module.php?m=estimate&amp;action=accessories&amp;switch_status=<?php echo $rsAccessories->id ?>&amp;product_id=<?php echo $iProductId ?>"
					...
					<li><a
						href="module.php?m=estimate&amp;action=accessory&amp;accessory_id=<?php echo $rsAccessories->id ?>&amp;product_id=<?php echo $iProductId ?>"
					...
					<li><a
						href="module.php?m=estimate&amp;action=accessories&amp;delete_accessory=<?php echo $rsAccessories->id ?>&amp;product_id=<?php echo $iProductId ?>"

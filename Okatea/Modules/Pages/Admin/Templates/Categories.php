...
<div id="rubriques_lists">
	...
	while ($rsCategories->fetch())
	{
		...
		if ($rsCategories->active)
		{
			echo '<a href="'.$view->generateUrl('Pages_categories').'?switch_status='.$rsCategories->id.'" '.
		...
		echo
		' - <a href="'.$view->generateUrl('Pages_category', array('category_id' => $rsCategories->id)).'" '.
		'title="'.$view->escapeHtmlAttr(sprintf(__('m_pages_cats_edit_%s'), $rsCategories->title)).'" '.

	...
	$okt->page->addButton('pagesCatsBtSt', array(
		...
		'ui-icon' => 'plusthick'
			...
			$okt->page->addButton('pagesCatsBtSt', array(
				...
				'url' 			=> $view->generateUrl('Pages_category', array('category_id' => $aCategoryData['cat']['id'])).'?switch_status=1',
			...
			$okt->page->addButton('pagesCatsBtSt', array(
				...
				'url' 			=> $okt->router->generateFromAdmin('pagesCategory', array('slug' => $aCategoryData['locales'][$okt->user->language]['slug']), null, true),
				'ui-icon' 		=> 'extlink'
			));
}
...
if ($aCategoryData['cat']['id'])
{
	$path = $okt->module('Pages')->categories->getPath($aCategoryData['cat']['id'], true, $okt->user->language);

	while ($path->fetch()) {
		$okt->page->addGlobalTitle($path->title, $view->generateUrl('Pages_category', array('category_id' => $path->id)));
	}
}
else {
...
# RTE
if ($okt->module('Pages')->config->categories['descriptions']) {
	$okt->page->applyRte($okt->module('Pages')->config->categories['rte'],'textarea.richTextEditor');
}
	...
	$okt->page->js->addReady('
		$("#sortable").sortable({
			...
			update: function(event, ui) {
				...
				$.ajax({
					data: result,
					url: "'.$view->generateUrl('Pages_category', array('category_id' => $aCategoryData['cat']['id'])).'?ajax_update_order=1",
...
<form action="<?php echo !empty($aCategoryData['cat']['id']) ? $view->generateUrl('Pages_category', array('category_id' => $aCategoryData['cat']['id'])) : $view->generateUrl('Pages_category_add'); ?>" method="post">
	...
	<div id="tabered">
		<ul>
			...
			<?php if ($okt->module('Pages')->config->enable_metas) : ?>
		...
		<div id="tab_options">
			<h3><?php _e('m_pages_cat_options_title') ?></h3>

			<div class="two-cols">
				<p class="field col"><label for="p_parent_id"><?php _e('m_pages_cat_parent') ?></label>
			...
			<div class="two-cols">
				<?php if (!empty($okt->module('Pages')->config->templates['list']['usables'])) : ?>

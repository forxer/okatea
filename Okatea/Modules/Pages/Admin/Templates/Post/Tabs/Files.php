...
<?php for ($i=1; $i<=$okt->module('Pages')->config->files['number']; $i++) : ?>
	<div class="col">
		...
		if (!empty($aPageData['files'][$i])) :
			...
			<p class="field" lang="<?php echo $aLanguage['code'] ?>"><label for="p_files_title_<?php echo $i ?>_<?php echo $aLanguage['code'] ?>"><?php $okt->languages->unique ? printf(__('m_pages_post_file_title_%s'), $i) : printf(__('m_pages_post_file_title_%s_in_%s'), $i, $aLanguage['title']) ?> <span class="lang-switcher-buttons"></span></label>
			...
			<p><a href="<?php echo $view->generateUrl('Pages_post', array('page_id' => $aPageData['post']['id'])) ?>?delete_file=<?php echo $i ?>"
			onclick="return window.confirm('<?php echo $view->escapeJs(_e('m_pages_post_delete_file_confirm')) ?>')"
			class="icon delete"><?php _e('m_pages_post_delete_file')?></a></p>
			<?php endif; ?>

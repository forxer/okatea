...
<?php for ($i=1; $i<=$okt->module('Pages')->config->images['number']; $i++) : ?>
	<div class="col">
		<fieldset>
			...
			<p class="field">
				<label for="p_images_<?php echo $i ?>"><?php printf(__('m_pages_page_image_%s'), $i) ?></label>
	...
	:
		...
		$aCurImageAlt = isset($aPageData['images'][$i]['alt']) ? $aPageData['images'][$i]['alt'] : [];
		$aCurImageTitle = isset($aPageData['images'][$i]['title']) ? $aPageData['images'][$i]['title'] : [];
		...
		?>
				...
				<label
					for="p_images_title_<?php echo $i ?>_<?php echo $aLanguage['code'] ?>"><?php $okt['languages']->hasUniqueLanguage() ? printf(__('m_pages_page_image_title_%s'), $i) : printf(__('m_pages_page_image_title_%s_in_%s'), $i, $aLanguage['title']) ?> <span
			...
			<p class="field" lang="<?php echo $aLanguage['code'] ?>">
				<label
					for="p_images_alt_<?php echo $i ?>_<?php echo $aLanguage['code'] ?>"><?php $okt['languages']->hasUniqueLanguage() ? printf(__('m_pages_page_image_alt_text_%s'), $i) : printf(__('m_pages_page_image_alt_text_%s_in_%s'), $i, $aLanguage['title']) ?> <span
					class="lang-switcher-buttons"></span></label>
				...
				<p>
				<a href="<?php echo $aPageData['images'][$i]['img_url']?>"
					rel="pages_images"
					title="<?php echo $view->escapeHtmlAttr(sprintf(__('m_pages_page_image_title_attr_%s'), $aPageData['locales'][$okt['visitor']->language]['title'], $i)) ?>"

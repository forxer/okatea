...
class CustomFields
{
	...
	public function getFields($aParams = [])
	{
		...
		$query = 'SELECT f.id, f.status, f.register_status, f.user_editable, f.type, f.ord, f.html_id, f.options, ' . 'fl.title, fl.description, fl.value ' . 'FROM ' . $this->t_fields . ' f ' . 'LEFT JOIN ' . $this->t_fields_locales . ' AS fl ON fl.field_id=f.id ' . 'WHERE 1 ' . $reqPlus . 'ORDER BY ord ASC ';
	...
	{
		$query = 'SELECT field_id, language, title, value, description ' . 'FROM ' . $this->t_fields_locales . ' ' . 'WHERE field_id=' . (integer) $iFieldId;
	...
	{
		$query = 'SELECT field_id, value ' . 'FROM ' . $this->t_fields_values . ' ' . 'WHERE user_id = ' . (integer) $iUserId;
	...
	protected function buildFieldHtmlId($title, $html_id, $iFieldId)
	{
		...
		$query = 'SELECT html_id FROM ' . $this->t_fields . ' ' . 'WHERE html_id=\'' . $this->db->escapeStr($html_id) . '\' ' . 'AND id <> ' . (integer) $iFieldId . ' ' . 'ORDER BY html_id DESC';
	...
	/**
	 * Indique de quelle forme est le type.
	 * Simple (Champ texte et Zone de texte)

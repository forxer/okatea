...
if (!empty($_GET['valide']) && $okt['visitor']->checkPerm('users_edit'))
{
	...
	if ($okt->users->updUser($upd_params))
	{
		...
		$oMail->useFile(__DIR__ . '/../../Locales/' . $edit_language . '/Templates/validate_user.tpl', array(
			...
			'SITE_URL' => $okt['request']->getSchemeAndHttpHost() . $okt['config']->app_url
		));
		...
		$okt['flashMessages']->success(__('m_users_validated_user'));
...
if (!empty($_POST['change_password']) && $okt['visitor']->checkPerm('change_password') && $okt['visitor']->checkPerm('users_edit'))
{
	...
	if ($okt->users->changeUserPassword($upd_params))
	{
		...
		$okt['flashMessages']->success(__('m_users_user_edited'));

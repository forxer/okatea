...
<div class="two-cols">
	<p id="new_buttons" class="col">
		<a
			href="configuration.php?action=theme_editor&amp;theme=<?php echo $sThemeId ?>&amp;new_file=1"
			id="new_file_button"><?php
	_e('c_a_te_new_file')?></a> <a
			href="configuration.php?action=theme_editor&amp;theme=<?php echo $sThemeId ?>&amp;new_template=1"
			id="new_tpl_button"><?php
	_e('c_a_te_new_tpl')?></a>
	</p>

	<div id="treecontrol" class="col right">
		<a title="Collapse the entire tree below" href="#"><img
			src="<?php echo $okt->options->public_url ?>/plugins/treeview/images/minus.gif" /> <?php _e('c_a_te_action_collapse_all') ?></a>
		<a title="Expand the entire tree below" href="#"><img
			src="<?php echo $okt->options->public_url ?>/plugins/treeview/images/plus.gif" /> <?php _e('c_a_te_action_expand_all') ?></a>
	</div>
</div>
<div class="ui-helper-clearfix">
	...
	<?php if ($sFilename) : ?>
		<form class="col" action="configuration.php" method="post">
			...
			<?php echo form::checkbox('make_backup', 1, true) ?><label
					for="make_backup"><?php _e('c_a_te_make_backup') ?></label>
			</p>
		</form>

		<?php
		
$aBackupFiles = $oThemeEditor->getBackupFiles();
		...
		<?php foreach ($aBackupFiles as $sBackupFile) : ?>
			<li><?php echo $sBackupFile?>
				- <a
				href="configuration.php?action=theme_editor&amp;theme=<?php echo $sThemeId ?>&amp;file=<?php echo rawurlencode($sFilename) ?>&amp;restore_backup=<?php echo rawurlencode($sBackupFile) ?>"
				onclick="return window.confirm('<?php echo html::escapeJS(__('c_a_te_action_restore_confirm')) ?>')"><?php _e('c_a_te_action_restore') ?></a>
				- <a
				href="configuration.php?action=theme_editor&amp;theme=<?php echo $sThemeId ?>&amp;file=<?php echo rawurlencode($sFilename) ?>&amp;delete_backup=<?php echo rawurlencode($sBackupFile) ?>"
				onclick="return window.confirm('<?php echo html::escapeJS(__('c_a_te_action_delete_confirm')) ?>')"><?php _e('c_a_te_action_delete') ?></a>
			</li>
		<?php endforeach; ?>
		</ul>
		<?php endif; ?>
...
$oThemeEditor->loadThemeFilesTree();
...
<ul>
	...
	<li><a
		href="configuration.php?action=theme_editor&amp;theme=<?php echo $aTheme['id'] ?>"><?php echo $aTheme['name'] ?></a></li>
	<?php endforeach; ?>
</ul>

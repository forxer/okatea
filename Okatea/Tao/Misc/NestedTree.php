...
class NestedTree
{
	...
	{
		$array = array_merge($this->fields, array(
			'nleft',
			'nright',
			'level'
		));
	...
	public function getDescendants($id = 0, $includeSelf = false, $childrenOnly = false)
	{
		...
		if ($childrenOnly)
		{
			if ($includeSelf)
			{
				$query = sprintf('select %s from %s where %s = %d or %s = %d order by nleft', $this->getFields(), $this->table, $this->fields['id'], $parent_id, $this->fields['parent'], $parent_id);
			}
			else
			{
				$query = sprintf('select %s from %s where %s = %d order by nleft', $this->getFields(), $this->table, $this->fields['parent'], $parent_id);
			}
		}
		else
		{
			...
			{
				$query = sprintf('select %s from %s where nleft >= %d and nright <= %d order by nleft', $this->getFields(), $this->table, $nleft, $nright);
			}
			elseif ($nleft > 0)
			{
				$query = sprintf('select %s from %s where nleft > %d and nright < %d order by nleft', $this->getFields(), $this->table, $nleft, $nright);
	...
	/**
	 * Check if one node descends from another node.

...
class Installer extends BaseInstaller
{
	...
	protected function preInstall()
	{
		...
		$this->checklist->addItem(
			'theme_file',
			file_exists($this->root() . '/Theme.php'),
			'Theme handler file exists',
			'Theme handler file doesn\'t exists'
		);
		...
		if ($this->checklist->checkItem('theme_file'))
		{
			...
			$this->checklist->addItem(
				...
				'Theme handler class "' . $sClassName . '" exists',
				'Theme handler class "' . $sClassName . '" doesn\'t exists'
			);

			$this->checklist->addItem(
				'theme_class_valid',
				is_subclass_of($sClassName, '\\Okatea\\Tao\\Extensions\\Themes\\Theme'),
				'Theme handler class "' . $sClassName . '" is a valid theme class',
				'Theme handler class "' . $sClassName . '" is not a valid theme class'
			);
		}

		return $this->checklist->checkItem('theme_file')
			&& $this->checklist->checkItem('theme_class')
			&& $this->checklist->checkItem('theme_class_valid');
	}

	public function compareFiles()
	{
		$this->getComparator()->folder($this->root() . '/Install/Assets/', $this->okt['public_path'] . '/themes/' . $this->id() . '/');
	}

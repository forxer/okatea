...
class Menus
{
	...
	public function getMenus(array $aParams = array())
	{
		...
		$sQuery = 'SELECT m.id, m.title, m.active, m.tpl, COUNT(i.id) AS num_items ' . 'FROM ' . $this->t_menus . ' AS m ' . 'LEFT JOIN ' . $this->t_items . ' AS i ON m.id=i.menu_id ' . 'WHERE 1 ' . $sReqPlus . ' ' . 'GROUP BY m.id ';
	...
	public function switchMenuStatus($iMenuId)
	{
		...
		$query = 'UPDATE ' . $this->t_menus . ' SET ' . 'active = 1-active ' . 'WHERE id=' . (integer) $iMenuId;
	...
	public function delMenu($iMenuId)
	{
		...
		# then, remove menu
		$sQuery = 'DELETE FROM ' . $this->t_menus . ' ' . 'WHERE id=' . (integer) $iMenuId;
	...
	public function getItems(array $aParams = array())
	{
		...
		$sQuery = 'SELECT i.id, i.menu_id, i.active, i.type, i.ord, il.title, il.url ' . 'FROM ' . $this->t_items . ' AS i ' . 'LEFT JOIN ' . $this->t_items_locales . ' AS il ON i.id=il.item_id ' . 'WHERE 1 ' . $sReqPlus;
...
*/
	...
	public function setItemStatus($iItemId, $iStatus)
	{
		...
		$query = 'UPDATE ' . $this->t_items . ' SET ' . 'active = ' . $iStatus . ' ' . 'WHERE id=' . (integer) $iItemId;

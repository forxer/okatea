...
class TemplatesSet
{
	...
	public function __construct($okt, $aConfig, $sBase, $sTplFamily, $sBaseUrl = '/')
	{
		...
		$this->okt['l10n']->loadFile($this->okt['locales_path'] . '/%s/admin/templates.config');
	...
	protected function loadTemplatesPaths()
	{
		...
		{
			$this->aTemplatesPath = (array) glob($this->okt['themes_path'] . '/DefaultTheme/Templates/' . $this->sBase . '/*/template.php');
		}
		
		# then, get current theme templates
		$aThemeTemplates = (array) glob($this->okt['themes_path'] . '/' . $this->okt['config']['themes']['desktop'] . '/Templates/' . $this->sBase . '/*/template.php');
	 ...
	 *
	 * @param string $sDir
	 *        	Chemin du répertoir du template
	 * @return array
	 */
	...
	public static function getThemeIdFromTplPath($sTplPath)
	{
		...
		return Utilities::getNextSubDir($sTplPath, $okt['themes_path']);
	}
	...
	public function getHtmlConfigUsablesTemplates($bUsableField = true)
	{
		...
		foreach ($this->getTplInfos() as $aTplInfos)
		{
			...
			'</td>' . '<td><p>' . $aTplInfos['version'] . '</p></td>' . '<td><p>' . $aTplInfos['author'] . '</p></td>' . '<td><p>' . $aTplInfos['theme'] . '</p></td>' . '<td class="center small"><p>' . form::radio(array(
				$this->sFormPrefix . 'tpl_default_' . $this->sTplFamily,
				$this->sFormPrefix . 'tpl_default_' . $this->sTplFamily . '_' . $aTplInfos['id']
			), $aTplInfos['id'], ($aTplInfos['id'] == $this->aConfig['default'])) . '</p></td>';
			
			if ($bUsableField)
			{
				$sReturn .= '<td class="center small">' . form::checkbox(array(
					$this->sFormPrefix . 'tpl_usables_' . $this->sTplFamily . '[]',
					$this->sFormPrefix . 'tpl_usables_' . $this->sTplFamily . '_' . $aTplInfos['id']
				), $aTplInfos['id'], in_array($aTplInfos['id'], $this->aConfig['usables'])) . '</td>';
			}
			
			$sReturn .= 

			'<td class="nowrap small">' . '<ul class="actions">';
			...
			$sReturn .= '<li>' . '<a href="' . $this->sBaseUrl . 'tpl_delete=1' . '&amp;tpl_family=' . rawurlencode($this->sTplFamily) . '&amp;tpl_id=' . rawurlencode($aTplInfos['id']) . '" ' . 'onclick="return window.confirm(\'' . Escaper::js(__('c_a_tpl_config_delete_confirm')) . '\')" ' . 'class="icon package_delete">' . __('c_c_action_Delete') . '</a>' . '</li>';

...
class Page
{
	...
	public function applyRte($retId, $sElement = null, array $aOptions = [])
	{
		...
		{
			call_user_func_array($this->rteList[$retId]['callback'], array(
				$sElement,
				$aOptions
			));
		}
	...
	public function applyLbl($lblId, $sElement = 'a.modal', $conteneur = ".modal-box", array $aOptions = [])
	{
		...
		{
			call_user_func_array($this->lblList[$lblId]['callback'], array(
				$sElement,
				$conteneur,
				$aOptions
			));
	 ...
	 *        	Force le scan des fichiers/n'utilise pas le pseudo cache
	 * @return array
	 */

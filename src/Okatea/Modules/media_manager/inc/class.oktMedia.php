...
//		$this->addExclusion(__DIR__.'/../');
	...
	*/
	...
	protected function fileRecord($rs)
	{
		...
		if (!$this->isFileExclude($this->root.'/'.$rs->media_file) && is_file($this->root.'/'.$rs->media_file))
		{
			...
			&& $this->okt->user->id != $f->media_user_id) {
				$f->del = false;
				$f->editable = false;
			}
	...
	*/
	public function getDir($type=null)
	{
		...
		'm.media_creadt, m.media_upddt, m.media_private, m.user_id, '.
		'u.username, u.lastname, u.firstname '.
		'FROM '.$this->t_media.' AS m '.
			'LEFT JOIN '.$this->t_users.' AS u ON u.id=m.user_id '.
		"WHERE media_path = '".$this->path."' ".
		"AND media_dir = '".$this->db->escapeStr($media_dir)."' ";
		...
		while ($rs->fetch())
		{
			...
			elseif (!empty($p_dir['files']) && $this->relpwd == '')
			{
				...
				$this->db->execute(
					'DELETE FROM '.$this->t_media.' '.
					"WHERE media_path = '".$this->db->escapeStr($this->path)."' ".
					"AND media_file = '".$this->db->escapeStr($rs->media_file)."' "
				);
	...
	*/
	public function getFile($id)
	{
		...
		'FROM '.$this->t_media.' AS m '.
			'LEFT JOIN '.$this->t_users.' AS u ON u.id=m.user_id '.
		"WHERE m.media_path = '".$this->path."' ".
		'AND m.media_id = '.(integer) $id.' ';
	...
	*/
	public function getPostMedia($post_id,$media_id=null)
	{
		...
		'FROM '.$this->t_media.' M '.
		'INNER JOIN '.$this->t_media_ref.' PM ON (M.media_id = PM.media_id) '.
		"WHERE media_path = '".$this->path."' ".
		'AND post_id = '.$post_id.' ';
	...
	*/
	public function createFile($name,$title=null,$private=false,$dt=null)
	{
		...
		'FROM '.$this->t_media.' '.
		"WHERE media_path = '".$this->db->escapeStr($this->path)."' ".
		"AND media_file = '".$this->db->escapeStr($media_file)."' ";
	...
	*/
	public function updateFile($file,$newFile)
	{
		...
		if (!$this->okt->checkPerm('media_admin')
		&& $this->okt->user->id != $file->media_user_id) {

...
class NestedTreei18n
{
	...
	public function __construct($okt, $sTable, $sTableLocales, $idField, $parentField, $sSortField, $sJoinField='category_id', $sLanguageField='language', $addFields=array(), $addLocalesFields=array())
	{
		...
		$this->aFields = array_merge($addFields, array('id' => $idField, 'parent' => $parentField));
	...
	{
		$fields = array_merge($this->aFields,array('nleft','nright','level'));

		if ($in_array) {
			return array_merge($fields,$this->aLocalesFields);
		}

		$fields = array_map(array($this,'prependTAlias'),$fields);
		$localesFields = array_map(array($this,'prependLAlias'),$this->aLocalesFields);
	...
	{
		return $this->sTable.' AS '.$this->sTablePrefix.' '.
	...
	public function getNode($id)
	{
		$query = sprintf('SELECT %s FROM %s WHERE %s = %d',
			$this->getFields(),
			$this->getFrom(),
			$this->sTablePrefix.'.'.$this->aFields['id'],
			$id
		);
	 ...
	 * @param   bool    $bIncludeSelf   Whether or not to include the passed node in the
	 *                                  the results. This has no meaning if fetching entire tree.
	 * @param   bool    $bChildrenOnly  True if only returning children data. False if
	 *                                  returning all descendant data
	 * @param   string  $sLanguageCode  The ISO  code to restrict results.
	...
	public function getDescendants($id=0, $bIncludeSelf=false, $bChildrenOnly=false, $sLanguageCode=null)
	{
		...
		if ($bChildrenOnly)
		{
			if ($bIncludeSelf)
			{
				$query = sprintf('SELECT %s FROM %s WHERE (%s = %d OR %s = %d) '.$sLanguageWhere.'ORDER BY nleft',
					$this->getFields(),
					$this->getFrom(),
					$this->aFields['id'],
					$parent_id,
				...
				$query = sprintf('SELECT %s FROM %s WHERE %s = %d '.$sLanguageWhere.'ORDER BY nleft',
					$this->getFields(),
					$this->getFrom(),
					$this->aFields['parent'],
					$parent_id
		...
		else
		{
			...
			{
				$query = sprintf('SELECT %s FROM %s WHERE nleft >= %d AND nright <= %d '.$sLanguageWhere.'ORDER BY nleft',
					$this->getFields(),
					$this->getFrom(),
					$nleft,
					$nright
			...
			elseif ($nleft > 0)
			{
				...
				$query = sprintf('SELECT %s FROM %s '.$sLanguageWhere.'ORDER BY nleft',
					$this->getFields(),
					$this->getFrom()
				);
			}
	...
	public function getPath($id=0, $bIncludeSelf=false, $sLanguageCode=null)
	{
		...
		$query = sprintf('SELECT %s FROM %s WHERE '.$sWhere.' ORDER BY level',
			$this->getFields(),
			$this->getFrom(),
			$node->nleft,
			$node->nright
	...
	/**
	 * Check if one node descends WHERE another node. If either node is not
	...
	public function numDescendants($id)
	{
		if ($id == 0)
		{
			$query = sprintf('SELECT COUNT(%s) AS num_descendants FROM %s',
				$this->aFields['id'],
				$this->sTable
			);
	...
	{
		$query = sprintf('SELECT COUNT(%s) AS num_children FROM %s WHERE %s = %d',
			$this->aFields['id'],
			$this->sTable,
			$this->aFields['parent'],
			$id

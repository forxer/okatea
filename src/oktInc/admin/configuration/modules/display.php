...
foreach ($aInstalledModules as $aModule)
{
	if (file_exists($aModule['root'].'/CHANGELOG'))
	{
		$okt->page->openLinkInDialog('#'.$aModule['id'].'_changelog_link',array(
			'title' => html::escapeJS($aModule['name_l10n']." CHANGELOG"),
			'width' => 730,
			'height' => 500
		));
	}
...
<div id="tab-installed">
	<h3><?php _e('c_a_modules_installed_modules') ?> (<?php echo oktModules::pluralizeModuleCount($iNumInstalledModules); ?>)</h3>
	...
	<table class="common">
		...
		foreach ($aInstalledModules as $aModule) :
			...
			if (file_exists($aModule['root'].'/READ_ME'))
			{
				$module_links[] = '<a href="configuration.php?action=modules&amp;show_readme='.$aModule['id'].'"'.
		...
		<tr>
			<td class="<?php echo $td_class ?> small">
				...
				<?php endif; ?>
					...
					<?php else: ?>
					<img src="<?php echo OKT_COMMON_URL ?>/img/admin/module.png" width="32" height="32" alt="" />
					<?php endif; ?>
				<?php if ($aModule['status']): ?>
					</a>
				<?php endif; ?>
...
<div id="tab-uninstalled">
	<h3><?php echo __('c_a_modules_uninstalled_modules').' ('.oktModules::pluralizeModuleCount(count($aUninstalledModules)).')' ?></h3>
	...
	<?php elseif (!empty($aModulesRepositories)) : ?>
		...
		<table class="common">
			...
			<tr>
				...
				<td class="<?php echo $td_class; ?> center"><a href="configuration.php?action=modules&amp;repository=<?php echo urlencode($repo_name) ?>&amp;module=<?php echo urlencode($module['id']) ?>" class="lazy-load"><?php _e('c_c_action_Add') ?></a></td>
...
<div id="tab-updates">
	...
	<table class="common">
		...
		<tbody>
		<?php foreach ($updatables as $updatable) :
			$td_class = $line_count%2 == 0 ? 'even' : 'odd';
			$line_count++; ?>
		<tr>
			<th scope="row" class="<?php echo $td_class; ?> fake-td">
			<?php echo html::escapeHTML($updatable['name']) ?>
			<?php echo !empty($updatable['info']) ? '<br />'.html::escapeHTML($updatable['info']) : ''; ?>
			</th>
			<td class="<?php echo $td_class; ?> center"><?php echo html::escapeHTML($updatable['repository']) ?></td>
			<td class="<?php echo $td_class; ?> center"><?php echo html::escapeHTML($updatable['version']) ?></td>
			<td class="<?php echo $td_class; ?> small nowrap"><a href="configuration.php?action=modules&amp;repository=<?php

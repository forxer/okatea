<?php
	 ...
	 * 
	 * 
	 * Par defaut et en mode fichier, la classe opére une rotation sur les logs.
	...
	class Log {
		 ...
		 * @param bool $booArch :
		 *	 Active ou Desactive le systéme de rotation par archivage.
		 */
		//+	 --------------------------------------------------------------------------------
		...
		private function openLogFile() {		
			//On vide la cache sur le systéme de fichier
			clearstatcache();
		 ...
		 * (supprime tous les enregistrement antérieurs à la date passée en paramétre)
		 ...
		 * (supprime tous les enregistrement antérieurs à la date passée en paramétre dans la table mysql de log)
		 * @param string $strDateTime : chaine au format datetime (Y-m-d H:i:s) 
		 ...
		 * (supprime tous les enregistrement antérieurs à la date passée en paramétre dans le fichier de log)
		 * @param string $strDateTime : chaine au format datetime (Y-m-d H:i:s) 
		 * 	limite pour la suppression des logs
		 * 
		 */
		private function purgeLogCurentFile($strDateTime) {		
			...
			while(!feof($this->objFile)) {			
				...
				if ( sizeof($arrLineContent) > 1 ) {
					...
					//si la ligne respecte les critéres de date on la stock
				...
				foreach($arrArchLineContent as $strLineArch){
					...
					if ( sizeof($arrArchColContent) > 1 ) {
						...
						if ($intTimeStampLine>=$intTimeStampDateIni && $intTimeStampLine<=$intTimeStampDateEnd) {
							...
							//On a dépassé notre critére de date de fin

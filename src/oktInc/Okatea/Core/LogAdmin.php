...
class LogAdmin
{
	...
	public function add($aParams=array())
	{
		...
		$query =
		'INSERT INTO '.$this->t_log.' ( '.
			'user_id, username, ip, date, type, component, code, message '.
		' ) VALUES ( '.
			(integer)$aParams['user_id'].', '.
			'\''.$this->db->escapeStr($aParams['username']).'\', '.
			'\''.$this->db->escapeStr($aParams['ip']).'\', '.
			'NOW(), '.
			(integer)$aParams['type'].', '.
			'\''.$this->db->escapeStr($aParams['component']).'\', '.
			(integer)$aParams['code'].', '.
			'\''.$this->db->escapeStr($aParams['message']).'\' '.
		'); ';

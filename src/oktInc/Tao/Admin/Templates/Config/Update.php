...
elseif (!$okt->request->query->has('update_db'))
{
	...
	echo '<p><a href="'.$sBaseSelfUrl.'&amp;update_db=1" class="icon database_refresh">'.
...
if ($okt->request->query->has('update_db'))
{
	...
	if ($oChecklist->checkAll())
	{
		echo '<p>'.__('c_a_update_database_successful').' '.
				'<a href="'.$sBaseSelfUrl.'">'.__('c_a_update_complete_update').'</a></p>';
	}
	else  {
...
elseif ($bDigestIsReadable && !$sStep)
{
	...
	if (!empty($aArchives))
	{
		...
		{
			echo '<p><label class="classic">'.form::radio(array('backup_file'), $view->escape($v)).' '.
			$view->escape($v).'</label></p>';
		}

		echo
		'<p><strong>'.__('c_a_update_backup_warning').'</strong> '.
		sprintf(__('c_a_update_should_not_revert_prior_%s'),end($aArchives)).
		'</p>'.
...
elseif ($sStep == 'done' && !$okt->error->hasError())
{
	...
	' <strong><a href="'.$sBaseSelfUrl.'">'.__('c_a_update_finish').'</a></strong>'.
	'</p>';
}

...
class ImageUpload
{
	 ...
	 * @var array
	 */
	protected static $aAllowedTypes	= array('image/gif', 'image/jpeg', 'image/pjpeg', 'image/png', 'image/x-png');
	 ...
	 *
	 * @param integer $iItemId		L'identifiant de l'élément
	 * @param string $sOutput		Le fichier de sortie
	 * @param string $sExtension	L'extension du fichier
	 * @return void
	 */
	public function buildThumbnails($iItemId, $sOutput, $sExtension=null)
	{
		...
		$this->imageResize(
			...
			$this->aConfig['width'],
			$this->aConfig['height'],
			$this->aConfig['watermark_file'],
			$this->aConfig['watermark_position']
		);
		...
		$this->imageResize(
			...
			'min-'.$this->aConfig['watermark_file'],
			$this->aConfig['watermark_position']
		);
		...
		$this->imageResize(
			...
			'min2-'.$this->aConfig['watermark_file'],
			$this->aConfig['watermark_position']
		);
		...
		$this->imageResize(
			...
			'min3-'.$this->aConfig['watermark_file'],
			$this->aConfig['watermark_position']
		);
		...
		$this->imageResize(
			...
			'min4-'.$this->aConfig['watermark_file'],
			$this->aConfig['watermark_position']
		);
		...
		$this->imageResize(
			...
			'min5-'.$this->aConfig['watermark_file'],
			$this->aConfig['watermark_position']
		);
		...
		$this->imageResize(
			...
			$this->aConfig['square_size'],
			'sq-'.$this->aConfig['watermark_file'],
			$this->aConfig['watermark_position']
		);
	}
	 ...
	 *
	 * @param string $sSourceFile 	Chemin du fichier source
	 * @param string $sOutput 		Chemin du fichier destination
	 * @param string $sResizeType 	Type de redimensionnement (ratio ou crop)
	 * @param integer $iWidth 		Largeur
	 * @param integer $iHeight 		Hauteur
	...
	{
		return self::getImagesInfos(
			$this->getCurrentUploadDir($iItemId),
			$this->getCurrentUploadUrl($iItemId),
			$aImagesDb,
			$this->aConfig['number']
		);
	}
	...
	{
		return self::getImagesFilesInfos(
			$this->getCurrentUploadDir($iItemId),
			$this->getCurrentUploadUrl($iItemId),
			$sImagesName
		);
	...
	public static function getImagesFilesInfos($sImagesPath, $sImagesUrl, $sImage)
	{
		...
		if (!empty($aImages))
		{
			$aImages = array_merge($aImages,
				self::getImageFileInfos($sImagesPath, $sImagesUrl, $sImage, 'min'));

			$aImages = array_merge($aImages,
				self::getImageFileInfos($sImagesPath, $sImagesUrl, $sImage, 'min2'));

			$aImages = array_merge($aImages,
				self::getImageFileInfos($sImagesPath, $sImagesUrl, $sImage, 'min3'));

			$aImages = array_merge($aImages,
				self::getImageFileInfos($sImagesPath, $sImagesUrl, $sImage, 'min4'));

			$aImages = array_merge($aImages,
				self::getImageFileInfos($sImagesPath, $sImagesUrl, $sImage, 'min5'));

			$aImages = array_merge($aImages,
				self::getImageFileInfos($sImagesPath, $sImagesUrl, $sImage, 'sq'));
		}

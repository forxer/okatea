...
# Configuration site
$oktConfig = new oktConfig(new SingleFileCache(OKT_GLOBAL_CACHE_FILE), OKT_CONFIG_PATH.'/conf_site');


$css_admin = $oktConfig->minify_css_admin;
$js_admin = $oktConfig->minify_js_admin;
$css_public = $oktConfig->minify_css_public;
$js_public = $oktConfig->minify_js_public;

function oktReplaceMinifyURLStrings(&$str)
{
	$aReplacements = util::getMinifyReplacements($GLOBALS['oktConfig']);

	$str = '/'.str_replace($aReplacements['aSearch'], $aReplacements['aReplace'], $str);
}

array_walk($css_admin, 'oktReplaceMinifyURLStrings');
array_walk($js_admin, 'oktReplaceMinifyURLStrings');
array_walk($css_public, 'oktReplaceMinifyURLStrings');
array_walk($js_public, 'oktReplaceMinifyURLStrings');


$oktMinifyConfig = array(
	'js_admin' => $js_admin,
	'css_admin' => $css_admin,
	'js_public' => $js_public,
...
if (!is_dir(OKT_CACHE_PATH.'/minify')) {
	files::makeDir(OKT_CACHE_PATH.'/minify');
}


$min_enableBuilder = false;
$min_cachePath = OKT_CACHE_PATH.'/minify';
$min_serveOptions['maxAge'] = 3600*12; // 12 hours

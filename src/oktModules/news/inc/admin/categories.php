...
if (!empty($_GET['switch_status']))
{
	try
	{
		...
		$okt->redirect('module.php?m=news&action=categories&switched=1');
	}
	catch (Exception $e) {
...
if (!empty($_GET['delete']))
{
	try
	{
		...
		$okt->redirect('module.php?m=news&action=categories&deleted=1');
	}
	catch (Exception $e) {
		$okt->error->set($e->getMessage());
	}
...
# Confirmations
$okt->page->messages->success('deleted',__('m_news_cat_deleted'));

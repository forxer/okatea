...
xmlns:atom="http://www.w3.org/2005/Atom">
	<channel>
		<title><?php echo $this->escape(util::getSiteTitle().' - '.$okt->news->getTitle()) ?></title>
		<link><?php echo $this->escape($okt->config->app_host.$okt->news->config->url) ?></link>
		<atom:link href="<?php echo $this->escape($okt->config->app_host.$okt->news->config->feed_url) ?>" rel="self" type="application/rss+xml"/>
		<description><?php echo $this->escape(util::getSiteDescription()) ?></description>
		<language><?php echo $this->escape($okt->config->language) ?></language>
	...
	while ($rsPostsList->fetch()) : ?>
		...
		<item>
			<title><?php echo $this->escape($rsPostsList->title) ?></title>
			<link><?php echo $this->escape($okt->config->app_host.$rsPostsList->url) ?></link>
			<!--
			...
			<dc:creator><?php echo $this->escape($rsPostsList->author) ?></dc:creator>

			<?php if ($okt->news->config->categories['enable'] && $rsPostsList->rubrique_name) : ?>
			<category><?php echo $this->escape($rsPostsList->rubrique_name) ?></category>
			<?php endif; ?>

			<description><?php echo $this->escape($rsPostsList->content) ?></description>

			<?php $image = $rsPostsList->getFirstImageInfo();
			if (!empty($image) && isset($image['square_url'])) : ?>
			<!--
			<enclosure url="<?php echo $this->escape($okt->config->app_host.$image['square_url']) ?>"

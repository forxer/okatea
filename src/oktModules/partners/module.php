...
class module_partners extends Module
{
	...
	public function getPartners($params=array(), $count_only=false)
	{
		...
		{
			$query =
			'SELECT COUNT(p.id) AS num_partners '.
			'FROM '.$this->t_partners.' AS p '.
				'LEFT JOIN '.$this->t_partners_locales.' AS pl ON pl.partner_id=p.id '.
				'LEFT JOIN '.$this->t_categories.' AS c ON p.category_id=c.id '.
				'LEFT JOIN '.$this->t_categories_locales.' AS cl ON cl.category_id=c.id '.
			'WHERE 1 '.
			$reqPlus;
		}
		else {
			$query =
			'SELECT p.id, p.active, p.category_id, p.name, p.logo, p.created_at, p.updated_at, '.
			'pl.description, pl.url, pl.url_title, '.
	...
	{
		return $this->getPartners(array('id'=>$id,'active'=>2));
	}
	...
	{
		$query =
		'SELECT language, description, url, url_title '.
		'FROM '.$this->t_partners_locales.' '.
		'WHERE partner_id='.(integer)$partner_id;
		...
		if (!empty($this->params))
		{
			...
			if ($this->config->chp_description > 0)
			{
				if ($this->config->chp_description == 2 && (empty($this->params['descriptions'][$this->okt->config->language]))) {
					$this->error->set(sprintf(__('m_partners_error_missing_default_language_description_%s'),
			...
			if ($this->config->chp_url > 0)
			{
				foreach ($this->params['urls'] as $k=>$v)
				{
					...
					{
						$this->error->set(sprintf(__('m_partners_error_invalid_url_%s'),
						$this->okt->languages->list[$k]['title']));
					}
				}

				if ($this->config->chp_url == 2 && empty($this->params['urls'][$this->okt->config->language])) {
					$this->error->set(sprintf(__('m_partners_error_missing_default_language_url_%s'),
					$this->okt->languages->list[$this->okt->config->language]['title']));
				}
			}
			else {
			...
			if ($this->config->chp_url_title > 0)
			{
				if ($this->config->chp_url_title == 2 && empty($this->params['urls_titles'][$this->okt->config->language])) {
					$this->error->set(sprintf(__('m_partners_error_missing_default_language_url_title_%s'),
					$this->okt->languages->list[$this->okt->config->language]['title']));
				}
			}
			else {
	...
	public function addPartner($params=array())
	{
		...
		') VALUES ( '.
			...
			(is_null($this->params['category_id']) ? 'NULL, ' : (integer)$this->params['category_id'].', ').
			(empty($this->params['name']) ? 'NULL,' : '\''.$this->db->escapeStr($this->params['name']).'\', ').
			$this->params['date'].', '.
			'NOW() '.','.
			(integer)($max_ord+1).
		'); ';
	...
	public function deletePartner($id)
	{
		...
		$query =
		'DELETE FROM '.$this->t_partners_locales.' '.
		'WHERE partner_id='.(integer)$id;
...
/* CATEGORIES */
	...
	public function getCategories($params=array(), $count_only=false)
	{
		...
		{
			$query =
			'SELECT COUNT(id) AS num_categories '.
			'FROM '.$this->t_categories.' AS r '.
			'WHERE 1 ';
		}
		else {
			$query =
			'SELECT c.id, c.active, c.ord, c.parent_id, c.level, '.
			'cl.language, cl.name, '.
			'COUNT(p.id) AS num_items '.
			'FROM '.$this->t_categories.' AS c '.
				'LEFT JOIN '.$this->t_categories_locales.' AS cl ON cl.category_id=c.id '.
				'LEFT JOIN '.$this->t_partners.' AS p ON c.id=p.category_id '.
			'WHERE 1 '.
			$reqPlus.' '.
			'GROUP BY c.id '.
			'ORDER BY nleft asc ';
	...
	public function getCategoryLocales($category_id)
	{
		...
		'WHERE category_id='.(integer)$category_id;
	...
	public function addCategory($active, $names, $parent_id=0)
	{
		...
		foreach($names as $lang => $name)
		{
			...
			{
				$query =
				'INSERT INTO '.$this->t_categories_locales.' ( '.
					'category_id, language, name '.
				') VALUES ( '.
					(integer)$new_id.', '.
					'\''.$this->db->escapeStr($lang).'\', '.
	...
	public function regenMinLogos()
	{
		...
		$rsPartners = $this->getPartners(array('active'=>2));
		...
		while ($rsPartners->fetch())
		{
			...
			if (!empty($aImages['img_name']))
			{
				...
				$aImagesList = array_merge(
					$aImages,
					$this->getLogoUpload()->buildImageInfos($rsPartners->id, $aImages['img_name'])
				);
			}
	...
	public function updLogos($partner_id, $aImage=null)
	{
		...
		$query =
		'UPDATE '.$this->t_partners.' SET '.
			'logo='.(!is_null($aImage) ? '\''.$this->db->escapeStr($aImage).'\'' : 'NULL').' '.
		'WHERE id='.(integer)$partner_id;
	...
	protected function setPartnerInter()
	{
		foreach ($this->okt->languages->list as $aLanguage)
		{
			...
			$query =
			'INSERT INTO '.$this->t_partners_locales.' '.
				'(partner_id,language,description,url,url_title) '.
			'VALUES ('.
				(integer)$this->params['id'].', '.
				'\''.$this->db->escapeStr($aLanguage['code']).'\', '.
				(empty($this->params['descriptions'][$aLanguage['code']]) ? 'NULL' : '\''.$this->db->escapeStr($this->params['descriptions'][$aLanguage['code']]).'\'').', '.
				(empty($this->params['urls'][$aLanguage['code']]) ? 'NULL' : '\''.$this->db->escapeStr($this->params['urls'][$aLanguage['code']]).'\'').', '.
				(empty($this->params['urls_titles'][$aLanguage['code']]) ? 'NULL' : '\''.$this->db->escapeStr($this->params['urls_titles'][$aLanguage['code']]).'\'').' '.
			') ON DUPLICATE KEY UPDATE '.
				'description='.(empty($this->params['descriptions'][$aLanguage['code']]) ? 'NULL' : '\''.$this->db->escapeStr($this->params['descriptions'][$aLanguage['code']]).'\'').', '.
				'url='.(empty($this->params['urls'][$aLanguage['code']]) ? 'NULL' : '\''.$this->db->escapeStr($this->params['urls'][$aLanguage['code']]).'\'').', '.
				'url_title='.(empty($this->params['urls_titles'][$aLanguage['code']]) ? 'NULL' : '\''.$this->db->escapeStr($this->params['urls_titles'][$aLanguage['code']]).'\'').' ';

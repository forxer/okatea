...
l10n::set(dirname(__FILE__).'/../../locales/'.$okt->user->language.'/admin.categories');
...
if (!empty($_REQUEST['category_id']))
{
	...
	{
		$okt->error->set(sprintf(__('m_pages_cat_%s_not_exists'),$iCategoryId));
		$iCategoryId = null;
	}
	else
	{
	...
	if ($okt->pages->categories->checkPostData($aCategoryData, $aCategoryLocalesData))
	{
		...
		if (!empty($iCategoryId))
		{
			try
			{
				...
				$okt->redirect('module.php?m=pages&action=categories&do=edit&category_id='.$iCategoryId.'&updated=1');
			}
			catch (Exception $e) {
				$okt->error->set($e->getMessage());
			}
		...
		else
		{
			try
			{
				...
				$okt->redirect('module.php?m=pages&action=categories&do=edit&category_id='.$iCategoryId.'&added=1');
			}
			catch (Exception $e) {
				$okt->error->set($e->getMessage());
			}
...
$okt->page->setButtonset('pagesCatsBtSt',array(
	...
	'buttons' => array(
		array(
			...
			'url' 			=> 'module.php?m=pages&amp;action=categories',
...
if ($iCategoryId)
{
	...
	$okt->page->addButton('pagesCatsBtSt',array(
		...
		'url' 			=> 'module.php?m=pages&amp;action=categories&amp;do=edit&amp;switch_status=1&amp;category_id='.$iCategoryId,
		'ui-icon' 		=> ($aCategoryData['active'] ? 'volume-on' : 'volume-off'),
		'active' 		=> $aCategoryData['active'],
	));
	# bouton de suppression
	$okt->page->addButton('pagesCatsBtSt',array(
		'permission' 	=> ($iCategoryNumPages == 0),
		'title' 		=> __('c_c_action_Delete'),
		'url' 			=> 'module.php?m=pages&amp;action=categories&amp;delete='.$iCategoryId,
		'ui-icon' 		=> 'closethick',
	...
	$okt->page->js->addReady('
		$("#sortable").sortable({
			...
			update: function(event, ui) {
				...
				$.ajax({
					data: result,
					url: "module.php?m=pages&action=categories&do=edit&category_id='.$iCategoryId.'&ajax_update_order=1",
...
# Confirmations
$okt->page->messages->success('added',__('m_pages_cat_added'));
$okt->page->messages->success('updated',__('m_pages_cat_updated'));

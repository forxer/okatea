...
class oktFilterWords extends oktSpamFilter
{
	...
	public function gui($url)
	{
		...
		'<fieldset><legend>'.__('m_antispam_Add_word').'</legend>'.
		...
		else
		{
			$res .=
			'<form action="'.html::escapeURL($url).'" method="post">'.
			'<fieldset><legend>' . __('m_antispam_List') . '</legend>'.
			...
			while ($rs->fetch())
			{
				...
				form::checkbox(array('swd[]'),$rs->rule_id,false).' '.
	...
	{
		$strReq =
		'SELECT rule_id FROM '.$this->table.' '.
		"WHERE rule_type = 'word' ".

...
<?php # début Okatea : champs requis pour la validation JS
...
<?php # début Okatea : jQuery UI dialog pour le formulaire d'ajout
$okt->page->js->addReady('
	$(function() {
...
<?php # debut : affichage du message de confirmation d'ajout d'une signature
if (!empty($_GET['added'])) : ?>

<div class="success_box">
	<p><?php _e('m_guestbook_thank_you') ?></p>

	<?php /* message si la validation avant publication est activée */
...
<p id="guestbook-form-control"><a href="#guestbook-add" class="sign-link"><?php _e('m_guestbook_sign_it') ?></a></p>
...
<div id="guestbook-add">
	...
	<form action="<?php echo $okt->guestbook->config->url ?>" id="guestbook-form" method="post">
		...
		<select name="note" id="note" size="1">
			<option value="nc"<?php if ($aSigData['note'] == 'nc') { echo ' selected="selected"'; } ?>>-NC-</option>
			<?php for ($i=0; $i<=20; $i++) {
		...
		<input name="valbout" type="submit" value="<?php _e('m_guestbook_submit') ?>" /></p>
...
<?php # debut : boucle sur les signatures à afficher
...
<?php # debut : affichage pagination
...
<?php # début affichage liens vers les autres langues

...
<form action="module.php" method="get" id="filters-form">
	<fieldset>
		...
		<input type="submit" name="<?php echo $okt->estimate->filters->getFilterSubmitName() ?>" value="<?php _e('c_c_action_display') ?>" />
		<a href="module.php?m=estimate&amp;action=index&amp;init_filters=1"><?php _e('c_c_reset_filters') ?></a></p>
...
<?php # Si on as des demandes de devis à afficher
...
<form action="module.php" method="post" id="estimates-list">
		...
		<tr class="<?php echo $rsEstimates->odd_even ?><?php if ($rsEstimates->status) : ?> disabled<?php endif; ?>">
			...
			<td class="<?php echo $rsEstimates->odd_even ?>">
				...
				<?php printf(__('From %s to %s'),
					dt::dt2str(__('%A, %B %d, %Y'), html::escapeHTML($rsEstimates->start_at)),
					dt::dt2str(__('%A, %B %d, %Y'), html::escapeHTML($rsEstimates->end_at))
				); ?>
				<?php endif; ?>
			</td>
			...
			<td class="<?php echo $rsEstimates->odd_even ?> small nowrap">
				<ul class="actions">
					...
					<a href="module.php?m=estimate&amp;action=index&amp;untreated=<?php echo $rsEstimates->id ?>"
					title="<?php printf(__('m_estimate_list_mark_%s_as_untreated'), $rsEstimates->id) ?>"
					class="icon tick"><?php _e('m_estimate_list_treated')?></a>
					<?php else : ?>
					<a href="module.php?m=estimate&amp;action=index&amp;treated=<?php echo $rsEstimates->id ?>"
	...
	<div class="two-cols">
		...
		<div class="col right"><p><?php _e('m_estimates_list_estimates_action')?>

...
class estimateAccessories
{
	...
	public function getAccessories($params=array(), $count_only=false)
	{
		...
		{
			$query =
			'SELECT COUNT(a.id) AS num_accessories '.
			'FROM '.$this->t_accessories.' AS a '.
				'LEFT JOIN '.$this->t_products.' AS p ON p.id=a.product_id '.
			'WHERE 1 '.
			$reqPlus;
		}
		else {
			$query =
			'SELECT a.id, a.product_id, a.active, a.title, '.
	...
	{
		return $this->getAccessories(array('id'=>$iAccessoryId,'active'=>$iActive));
	}
	...
	public function delAccessory($id)
	{
		...
		$query =
		'DELETE FROM '.$this->t_accessories.' '.
		'WHERE id='.(integer)$id;

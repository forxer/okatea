...
elseif ($okt->page->action === 'gallery') {
	require __DIR__.'/inc/admin/gallery.php';
}
elseif ($okt->page->action === 'items') {
	require __DIR__.'/inc/admin/items.php';
}
...
elseif ($okt->page->action === 'add_zip' && $okt->galleries->config->enable_zip_upload && $okt->checkPerm('galleries_add')) {
	require __DIR__.'/inc/admin/add_zip.php';
}
elseif ($okt->page->action === 'add_multiples' && $okt->galleries->config->enable_multiple_upload && $okt->checkPerm('galleries_add') && file_exists(__DIR__.'/inc/admin/add_multiples/'.$okt->galleries->config->multiple_upload_type.'.php')) {
	require __DIR__.'/inc/admin/add_multiples/'.$okt->galleries->config->multiple_upload_type.'.php';
}

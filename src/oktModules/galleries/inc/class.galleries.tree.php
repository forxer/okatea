...
class galleriesTree extends nestedTreei18n
{
	 ...
	 * @param boolean $bCountOnly 	Ne renvoi qu'un compte de galeries
	...
	public function getGalleries($aParams=array(), $bCountOnly=false)
	{
		...
		{
			$sQuery =
			'SELECT COUNT(r.id) AS num_galleries '.
			'FROM '.$this->t_galleries.' AS r '.
				'LEFT JOIN '.$this->t_galleries_locales.' AS rl ON r.id=rl.gallery_id '.
				'LEFT JOIN '.$this->t_items.' AS p ON r.id=p.gallery_id '.
			'WHERE 1 '.
			$sReqPlus.' ';
		}
		else
		{
			$sQuery =
			'SELECT r.*, rl.*, COUNT(p.id) AS num_items '.
			'FROM '.$this->t_galleries.' AS r '.
				'LEFT JOIN '.$this->t_galleries_locales.' AS rl ON r.id=rl.gallery_id '.
				'LEFT JOIN '.$this->t_items.' AS p ON r.id=p.gallery_id '.
			'WHERE 1 '.
	...
	public function getGalleryI18n($iGalleryId)
	{
		$query =
		'SELECT * '.
		'FROM '.$this->t_galleries_locales.' '.
		'WHERE gallery_id='.(integer)$iGalleryId;
	...
	protected function setGallerySlug($iGalleryId, $sLanguage)
	{
		...
		$query =
		'SELECT slug FROM '.$this->t_galleries_locales.' '.
		'WHERE slug=\''.$this->db->escapeStr($sSlug).'\' '.
		'AND gallery_id <> '.(integer)$iGalleryId.' '.
		...
		$query =
		'UPDATE '.$this->t_galleries_locales.' SET '.
	...
	public function setGalleryPosition($iGalleryId, $iOrder)
	{
		...
		$sQuery =
		'UPDATE '.$this->t_galleries.' SET '.
			'ord='.(integer)$iOrder.' '.
		'WHERE id='.(integer)$iGalleryId;
	...
	public function deleteGallery($iGalleryId)
	{
		...
		$query =
		'DELETE FROM '.$this->t_galleries_locales.' '.
		'WHERE gallery_id='.(integer)$iGalleryId;
	...
	{
		$o = new oktImageUpload($this->okt, $this->okt->galleries->config->images_gal);
		$o->setConfig(array(
	...
	public function regenMinImages()
	{
		...
		$rsGalleries = $this->getGalleries(array('active'=>2));
		...
		while ($rsGalleries->fetch())
		{
			...
			if (!empty($aImages['img_name']))
			{
				...
				$aImagesList = array_merge(
					$aImages,
					$this->getImageUploadInstance()->buildImageInfos($rsGalleries->id, $aImages['img_name'])
				);
			}

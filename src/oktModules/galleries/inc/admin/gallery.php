...
l10n::set(dirname(__FILE__).'/../../locales/'.$okt->user->language.'/admin.gallery');
...
$aAllowedParents = array(__('m_galleries_gallery_first_level')=>0);
...
<form action="module.php" method="post" enctype="multipart/form-data">
	...
	<div id="tabered">
		...
		<div id="tab_gallery">
			...
			<p class="field" lang="<?php echo $aLanguage['code'] ?>"><label for="p_title_<?php echo $aLanguage['code'] ?>" title="<?php _e('c_c_required_field') ?>" class="required"><?php $okt->languages->unique ? _e('m_galleries_gallery_title') : printf(__('m_galleries_gallery_title_in_%s'),$aLanguage['title']) ?> <span class="lang-switcher-buttons"></span></label>
			...
			<p class="field" lang="<?php echo $aLanguage['code'] ?>"><label for="p_content_<?php echo $aLanguage['code'] ?>"><?php $okt->languages->unique ? _e('m_galleries_gallery_description') : printf(__('m_galleries_gallery_description_in_%s'),$aLanguage['title']) ?> <span class="lang-switcher-buttons"></span></label>
		...
		<div id="tab_image">
			...
			<p class="field"><label for="p_image"><?php _e('m_galleries_gallery_image_file') ?></label>
			...
			if (!empty($aGalleryData['image'])) :
				...
				<p class="field" lang="<?php echo $aLanguage['code'] ?>"><label for="p_images_title_1_<?php echo $aLanguage['code'] ?>"><?php $okt->languages->unique ? __('m_galleries_gallery_image_title') : printf(__('m_galleries_gallery_image_title_in_%s'), $aLanguage['title']) ?> <span class="lang-switcher-buttons"></span></label>
				...
				<p class="field" lang="<?php echo $aLanguage['code'] ?>"><label for="p_images_alt_1_<?php echo $aLanguage['code'] ?>"><?php $okt->languages->unique ? __('m_galleries_gallery_image_alt_text') : printf(__('m_galleries_gallery_image_alt_text_in_%s'), $aLanguage['title']) ?> <span class="lang-switcher-buttons"></span></label>
		...
		<div id="tab_options">
			...
			<div class="two-cols">
				<p class="field col"><label for="p_parent_id"><?php _e('m_galleries_gallery_parent') ?></label>
				...
				<p class="field col"><label for="p_password"><?php _e('m_galleries_gallery_password')?></label>
				<?php echo form::text('p_password', 50, 255, html::escapeHTML($aGalleryData['db']['password'])) ?>
				<span class="note"><?php _e('m_galleries_gallery_password_empty') ?></span></p>
				<?php endif; ?>
			</div>

			<div class="two-cols">
				<p class="field col"><label><?php echo form::checkbox('p_active', 1, $aGalleryData['db']['active']) ?> <?php _e('c_c_status_Online') ?></label></p>

				<?php if ($okt->user->is_superadmin) : ?>
				<p class="field col"><label><?php echo form::checkbox('p_locked', 1, $aGalleryData['db']['locked']) ?> <?php _e('m_galleries_gallery_locked') ?></label></p>
				<?php endif; ?>
			</div>

			<div class="two-cols">
				<?php if (!empty($okt->galleries->config->templates['list']['usables'])) : ?>
				<p class="field col"><label for="p_tpl"><?php _e('m_galleries_gallery_tpl') ?></label>
				<?php echo form::select('p_tpl', $aTplChoices, $aGalleryData['db']['tpl'])?></p>
				<?php endif; ?>

				<?php if (!empty($okt->galleries->config->templates['item']['usables'])) : ?>
				<p class="field col"><label for="p_items_tpl"><?php _e('m_galleries_gallery_items_tpl') ?></label>
...
<form action="module.php" method="post">
	<div id="tab_siblings">
		...
		while ($aGalleryData['siblings']->fetch()) : ?>
			<li id="ord_<?php echo $aGalleryData['siblings']->id; ?>" class="ui-state-default"><label for="order_<?php echo $aGalleryData['siblings']->id ?>">

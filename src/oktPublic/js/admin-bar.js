...
if ( typeof(jQuery) != 'undefined' ) {
	...
	jQuery(document).ready(function($){
		var adminbar = $('#oktadminbar'), refresh, touchOpen, touchClose, disableHoverIntent = false;
		...
		touchClose = function() {
			...
			$(document.body).on( mobileEvent+'.wp-mobile-hover', function(e) {
				if ( !$(e.target).closest('#oktadminbar').length )
					adminbar.find('li.menupop.hover').removeClass('hover');
			});
		};
		...
		$('#oktadminbar li.menupop > .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 13 )
				return;
		...
		$('#oktadminbar .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 27 )
				return;
		...
		$('#oktadminbar').click( function(e) {
			if ( e.target.id != 'oktadminbar' )
				return;
	...
	(function(d, w) {
		...
		scrollToTop = function(t) {
			...
			// Ensure that the #oktadminbar was the target of the click.
			if ( t.id != 'oktadminbar' && t.id != 'wp-admin-bar-top-secondary' )
				return;
		...
		addEvent(w, 'load', function() {
			aB = d.getElementById('oktadminbar');
